...
b.push(a[c]);return b},savePublicKeys:function(a){for(var b in a){var c=app.activeUser.publicKeys;null==c&&(c={});var d={},e;for(e in a)a[e].sender.id_str!=app.activeUser.user.id&&(d[a[e].sender.id_str]||(d[a[e].sender.id_str]={key:a[e].seecret,screen_name:a[e].sender.screen_name,id_str:a[e].sender.id_str,receiveDate:a[e].created_at}));for(var f in d)c[f]=d[f];app.saveObject("publicKeys-"+app.activeUser.user.id_str,c);app.activeUser.publicKeys=c}},processDirectMessagesResponse:function(a){1==a.length&&
a[0].id_str==app.dmMaxId?$("#directMessages").append(Handlebars.templates["no-more-direct-messges-template.hbs"]()):(a=app.processDirectMessages(a,app.getUniqueDMSenders),app.timelineContainsKeys(a)&&app.savePublicKeys(app.getPublicKeyMessagesFromDMList(a)),app.timelineContainsEncryptedMessages(a)?(app.markEncryptedDirectMessages(a),console.log("decrypting direct messages!"),app.decryptDirectMessages(a)):app.updateDirectMessagesUI(a))},markEncryptedDirectMessages:function(a){for(var b in a)this.isPGPMessage(a[b].seecret)&&
(a[b].encrypted=!0,a[b].decrypted=!1)},sendPublicKeyMessages:function(a,b){app.postDirectMessages(a,function(){app.trackSentPublicKeys(b)})},trackSentPublicKeys:function(a){var b=app.activeUser.sentPublicKeys;null==b&&(b={});for(var c in a)b[c]=new Date;app.activeUser.sentPublicKeys=b;app.saveObject("sentPublicKeys-"+app.activeUser.user.id_str,b)},passphrase:null,decryptDirectMessages:function(a){var b=openpgp.key.readArmored(app.activeUser.privateKey.armored).keys[0];app.decryptPrivateKey(b)?app.verifyMessagesDecrypted(a,
b):app.updateDirectMessagesUI(a)},verifyMessagesDecrypted:function(a,b){var c=null,d;for(d in a)if(a[d].encrypted&&0==a[d].decrypted){c=d;break}var e=app.activeUser.user;if(null!=c)if(a[c].sender.id_str==e.id)a[c].decrypted=!0,app.verifyMessagesDecrypted(a,b);else{a[d].decrypted=!0;a[d].decryptionSuccessful=!1;a[d].originaltext=a[d].text;try{var f=openpgp.message.readArmored(a[c].seecret),g={privateKey:b,message:f};if(app.activeUser.publicKeys&&app.activeUser.publicKeys[a[c].sender.id_str]){var l=
...
c.id_str,f);app.saveObject("publicKey-"+c.id_str,g);app.checkDirectMessageStatus();app.refreshActiveUserTotal();d()},setDirectMessageReceiver:function(a,b){$("#selectedReceiverId").val(a);$("#selectedReceiverName").val(b);var c={};c.name=b;c.id_str=a;var d=app.activeUser.publicKeys,e;for(e in d)d[e].id_str==a&&(c.publicKey=d[e],c.publicKey.displayDate=(new moment(c.publicKey.receiveDate)).format("MMMM Do YYYY, h:mm:ss a"));app.updateDirectMesssagesBreadcrumbs(c);app.checkDirectMessageStatus();$(".dmMessageDiv").hide();
$(".dm_"+a).fadeIn()},showAllDirectMessages:function(){$(".dmMessageDiv").show();app.updateDirectMesssagesBreadcrumbs();$("#directMessagesActionsContainer").html("")},updateDirectMesssagesBreadcrumbs:function(a){$("#directMessagesBreadCrumbContainer").html(Handlebars.templates["direct-message-breadcrumb-template.hbs"](a))},initiateDirectMessages:function(a,b){var c=app.generatePublicKeyMessages(a),d=[],e;for(e in c)d.push({receiverId:a,text:c[e]});b||d.push({text:app.joinMessage,receiverId:a});app.postDirectMessages(d,
function(){var c={};c[a]=!0;app.trackSentPublicKeys(c);app.checkDirectMessageStatus();b&&alert("Sent!")})},joinMessage:"I'm using Seecret to send encrypted direct messages over Twitter. Try it with me! Just go to https://www.seecret.net",prepareDirectMessage:function(a){var b=$("#directMessage").val();app.encryptDirectMessage(b,a)},decryptPrivateKey:function(a){var b=app.activeUser.keyOptions,c=null;app.passphrase&&(c=app.passphrase);b.passphrase&&(c=b.passphrase);b.passphrase||null!=app.passphrase||
...
app.getUserTimeline(c,app.handleTimelineResponse)},getTimeline:function(a,b){app.oauthResult.get("1.1/statuses/home_timeline.json",a).done(function(a){b(a);app.hideOverlays()}).fail(app.handleGetTimelineError)},getUserTimeline:function(a,b){app.oauthResult.get("1.1/statuses/user_timeline.json",a).done(function(a){b(a);app.hideOverlays()}).fail(app.handleGetTimelineError)},handleTimelineResponse:function(a){0<a.length?1==a.length&&a[0].id_str==app.maxId?$("#status-list").append(Handlebars.templates["no-more-timeline-message-template.hbs"]()):
...
d;for(d in a)if(a[d].seecret_envelope){var e=a[d].seecret_envelope,f=app.seecret_engine.getSeecretFromEnvelope(e),e=app.seecret_engine.getContentTypeFromEnvelope(e),f=app.seecret_engine.unhide(f,e);if(e==app.seecret_engine.config.CONTENT_TYPES.NUMBERS_ARRAY)try{f=seecret_compression.decompress(f),a[d].seecret=f}catch(g){a[d].seecret="Could not decompress the Seecret",a[d].seecret_error=g}else a[d].seecret=f;c.push(a[d])}else b&&!app.seecret_engine.hasSeecretContent(a[d].text)&&c.push(a[d]);return c},
getEarliestIncompleteChainStartId:function(a,b){for(var c in b)app.getFirstIncompleteChainStart(a,b[c])},getEarliestIncompleteDMChainStartId:function(a,b){for(var c in b)app.getFirstIncompleteDMChainStart(a,b[c])},getFirstIncompleteChainStart:function(a,b){var c=0,d;for(d in a)a[d].user.id_str==b&&app.seecret_engine.hasSeecretContent(a[d].text)&&(app.seecret_engine.isEnvelopeStart(a[d].text)&&(c=d),app.seecret_engine.isEnvelopeEnd(a[d].text)&&(c=0));return c},getFirstIncompleteDMChainStart:function(a,
...
!0)},processTimelineMessages:function(a,b){var c=b(a),d=app.getMessageIndexes(a,c,app.tweetSenderGetter);a=app.dechainifyTweets(a,d);app.maxId=app.lastProcessedStatusId=a[a.length-1].id_str;c=app.getEarliestIncompleteChainStartId(a,c);0<c&&(app.maxId=a[c].id_str);return a=app.unhideAndDecompressTimelineSeecrets(a,!document.getElementById("showOnlySeecrets").checked)},processTimelineWithFollowerInfo:function(a){a=app.processTimelineMessages(a,app.getUniqueTweeters);0<a.length?$("#status-list").append(Handlebars.templates["status-template.hbs"](a)):
