...
c);app.activeUser.publicKeys=c}},processDirectMessagesResponse:function(a){1==a.length&&a[0].id_str==app.dmMaxId?$("#directMessages").append(Handlebars.templates["no-more-direct-messages-template.hbs"]()):(a=app.processDirectMessages(a,app.getUniqueDMSenders),app.timelineContainsKeys(a)&&app.savePublicKeys(app.getPublicKeyMessagesFromDMList(a)),app.timelineContainsEncryptedMessages(a)?(app.markEncryptedDirectMessages(a),console.log("decrypting direct messages!"),app.decryptDirectMessages(a)):app.updateDirectMessagesUI(a))},
...
a&&(c.publicKey=d[e],c.publicKey.displayDate=(new moment(c.publicKey.receiveDate)).format("MMMM Do YYYY, h:mm:ss a"));app.updateDirectMesssagesBreadcrumbs(c);app.checkDirectMessageStatus();$(".dmMessageDiv").hide();$(".dm_"+a).fadeIn()},showAllDirectMessages:function(){$(".dmMessageDiv").show();app.updateDirectMesssagesBreadcrumbs();$("#directMessagesActionsContainer").html("")},updateDirectMesssagesBreadcrumbs:function(a){$("#directMessagesBreadCrumbContainer").html(Handlebars.templates["direct-message-breadcrumb-template.hbs"](a))},
initiateDirectMessages:function(a,b){var c=app.generatePublicKeyMessages(a),d=[],e;for(e in c)d.push({receiverId:a,text:c[e]});b||d.push({text:app.joinMessage,receiverId:a});app.postDirectMessages(d,function(){var c={};c[a]=!0;app.trackSentPublicKeys(c);app.checkDirectMessageStatus();b&&alert("Sent!")})},joinMessage:"I'm using Seecret to send encrypted direct messages over Twitter. Try it with me! Just go to https://www.seecret.net",prepareDirectMessage:function(a){var b=$("#directMessage").val();
app.encryptDirectMessage(b,a)},decryptPrivateKey:function(a){var b=app.activeUser.keyOptions,c=null;app.passphrase&&(c=app.passphrase);b.passphrase&&(c=b.passphrase);b.passphrase||null!=app.passphrase||(app.passphrase=c=prompt("Enter your passphrase"));return a.decrypt(c)},encryptDirectMessage:function(a,b){var c=app.activeUser.publicKeys,d=null,e;for(e in c)e==b&&(d=c[e].key);d&&(c=openpgp.key.readArmored(d).keys,c={publicKeys:c,data:a},app.activeUser.privateKey&&(d=openpgp.key.readArmored(app.activeUser.privateKey.armored),
...
b(a),d=app.getSeecretMessageIndexesBySender(a,c);a=app.dechainifyDirectMessages(a,d);app.dmMaxId=app.lastProcessedStatusId=a[a.length-1].id_str;c=app.getEarliestIncompleteDMChainStartId(a,c);0<c&&(app.dmMaxId=a[c].id_str);return a=app.unhideAndDecompressTimelineSeecrets(a,!0)},processTimelineMessages:function(a,b){var c=b(a),d=app.getMessageIndexes(a,c,app.tweetSenderGetter);a=app.dechainifyTweets(a,d);app.maxId=app.lastProcessedStatusId=a[a.length-1].id_str;c=app.getEarliestIncompleteChainStartId(a,
c);0<c&&(app.maxId=a[c].id_str);return a=app.unhideAndDecompressTimelineSeecrets(a,!document.getElementById("showOnlySeecrets").checked)},processTimelineWithFollowerInfo:function(a){a=app.processTimelineMessages(a,app.getUniqueTweeters);0<a.length?$("#status-list").append(Handlebars.templates["status-template.hbs"](a)):$("#showOnlySeecrets")&&$("#status-list").append(Handlebars.templates["no-seecrets-in-timeline-segment-template.hbs"]());$(".basicTweet").fadeIn();app.updatingTimeline=!1;app.userTimelineId?
...
$("#privateKeyPassphrase").val("");$("#privateKeyCopyarea").empty();$("#publicKeysDisplayArea").empty();app.unauthenticatedHeader();app.hideAllViews();$("#privateMessageButton").hide();$("#timelineButton").hide();app.unauthenticatedMenu();app.setView(app.ABOUT_VIEW)},authenticatedMenu:function(){$("#privateMessageButton").show();$("#timelineButton").show();$("#settingsButton").show();$("#logoutButton").show();$("#hashesButton").show();$("#publicKeysButton").show();$("#aboutButton").show()},unauthenticatedMenu:function(){$("#privateMessageButton").hide();
