...
app.setView(app.HASHES_VIEW),app.unauthenticatedMenu();else{var b=/^SHA1\((.*)\)$/,c=[],d;for(d in a)if(b.test(a[d].name)){var e=b.exec(a[d].name);a[d].name=e[1].replace("../","");c.push(a[d])}$("#hashesContent").html(Handlebars.templates["hashes-template.hbs"](c));app.initialize()}}).fail(function(a){$("#hashesContent").html(Handlebars.templates["hashes-not-found-template.hbs"]());app.setView(app.HASHES_VIEW);app.unauthenticatedMenu()})});
