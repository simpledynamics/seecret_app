...
updateUIBasedUponOauthResult:function(){null!=app.oauthResult?(app.authenticatedUI(),app.maxId=null,app.updateHomeTimeline()):app.unauthenticatedUI()},dmMaxId:null,getDirectMessages:function(a){app.updatingDirectMessages=!0;$("#directMessagesActionsContainer").empty();$("#directMessagesBreadCrumbContainer").empty();$("#friendName").empty();a||($("#directMessages").empty(),app.dmMaxId=null);var b={count:200,full_text:!0};app.dmMaxId&&a&&(b.max_id=app.dmMaxId);app.overlay();app.oauthResult.get("1.1/direct_messages.json",
{data:b}).done(function(a){0<a.length?(app.processDirectMessagesResponse(a),app.updateDirectMesssagesBreadcrumbs(null)):(app.hideOverlays(),$("#directMessages").html("No direct messages."))}).fail(function(a){app.hideOverlays()})},updateDirectMessagesUI:function(a){app.hideOverlays();$("#getMessagesButton2").remove();$("#directMessages").append(Handlebars.templates["direct-messages-template.hbs"](a));$("#olderDirectMessagesButtonContainer").show();app.updatingDirectMessages=!1},publicKeyStartRegex:/^\s*-----BEGIN PGP PUBLIC KEY BLOCK-----\s/,
...
return!1},getPublicKeyMessagesFromDMList:function(a){var b=[],c;for(c in a)app.isPublicKey(a[c].seecret)&&b.push(a[c]);return b},savePublicKeys:function(a){var b=app.activeUser.publicKeys;null==b&&(b={});var c={},d;for(d in a)a[d].sender.id_str==app.activeUser.user.id||c[a[d].sender.id_str]||(console.log("Got a key from "+a[d].sender.screen_name+" dated "+a[d].created_at),c[a[d].sender.id_str]={key:a[d].seecret,screen_name:a[d].sender.screen_name,id_str:a[d].sender.id_str,receiveDate:a[d].created_at});
for(var e in c)b[e]=c[e];app.saveObject("publicKeys-"+app.activeUser.user.id_str,b);app.activeUser.publicKeys=b},processDirectMessagesResponse:function(a){if(1==a.length&&a[0].id_str==app.dmMaxId)$("#directMessages").append(Handlebars.templates["no-more-direct-messages-template.hbs"]()),app.hideOverlays(),app.updatingDirectMessages=!1;else if(0==a.length)$("#directMessages").append(Handlebars.templates["no-seecrets-in-direct-messages-template.hbs"]()),app.hideOverlays(),app.updatingDirectMessages=
!1;else{a=app.processDirectMessages(a,app.getUniqueDMSenders);for(var b in a);app.timelineContainsKeys(a)&&app.savePublicKeys(app.getPublicKeyMessagesFromDMList(a));app.timelineContainsEncryptedMessages(a)?(app.markEncryptedDirectMessages(a),app.decryptDirectMessages(a)):app.updateDirectMessagesUI(a)}},markEncryptedDirectMessages:function(a){for(var b in a)this.isPGPMessage(a[b].seecret)&&(a[b].encrypted=!0,a[b].decrypted=!1)},sendPublicKeyMessages:function(a,b){app.postDirectMessages(a,function(){app.trackSentPublicKeys(b)})},
trackSentPublicKeys:function(a){var b=app.activeUser.sentPublicKeys;null==b&&(b={});for(var c in a)b[c]=new Date;app.activeUser.sentPublicKeys=b;app.saveObject("sentPublicKeys-"+app.activeUser.user.id_str,b)},passphrase:null,decryptDirectMessages:function(a){var b=openpgp.key.readArmored(app.activeUser.privateKey.armored).keys[0];app.decryptPrivateKey(b)?app.verifyMessagesDecrypted(a,b):app.updateDirectMessagesUI(a)},verifyMessagesDecrypted:function(a,b){var c=null,d;for(d in a)if(a[d].encrypted&&
0==a[d].decrypted){c=d;break}var e=app.activeUser.user;if(null!=c)if(a[c].sender.id_str==e.id)a[c].decrypted=!0,app.verifyMessagesDecrypted(a,b);else{a[d].decrypted=!0;a[d].decryptionSuccessful=!1;a[d].originaltext=a[d].text;try{var f=openpgp.message.readArmored(a[c].seecret),g={privateKey:b,message:f};if(app.activeUser.publicKeys&&app.activeUser.publicKeys[a[c].sender.id_str]){var l=openpgp.key.readArmored(app.activeUser.publicKeys[a[c].sender.id_str].key);g.publicKeys=l.keys}try{openpgp.decrypt(g).then(function(b){a[d].decryptionSuccessful=
...
a.getSeconds()+":"+a.getMilliseconds()),openpgp.generateKey(e).then(function(a){app.generatePrivateKeyPairSuccess(a,e,b,c)}))},generatePrivateKeyPairSuccess:function(a,b,c,d){var e=new Date;console.log("done generating key pair: "+e.getSeconds()+":"+e.getMilliseconds());c||delete b.passphrase;c=app.activeUser.user;var e=new Date,f={};f.armored=a.privateKeyArmored;f.dateCreated=e;var g={};g.armored=a.publicKeyArmored;g.dateCreated=e;app.saveObject("keyOptions-"+c.id_str,b);app.saveObject("privateKey-"+
c.id_str,f);app.saveObject("publicKey-"+c.id_str,g);app.checkDirectMessageStatus();app.refreshActiveUserTotal();d()},setDirectMessageReceiver:function(a,b){$("#selectedReceiverId").val(a);$("#selectedReceiverName").val(b);var c={};c.name=b;c.id_str=a;var d=app.activeUser.publicKeys,e;for(e in d)d[e].id_str==a&&(c.publicKey=d[e],c.publicKey.displayDate=(new moment(c.publicKey.receiveDate)).format("MMMM Do YYYY, h:mm:ss a"));$("#friendName").html(Handlebars.templates["direct-message-friend-name.hbs"](c));
app.checkDirectMessageStatus();$(".dmMessageDiv").hide();$(".dm_"+a).fadeIn()},updateDirectMesssagesBreadcrumbs:function(a){$("#directMessagesBreadCrumbContainer").html(Handlebars.templates["direct-message-breadcrumb-template.hbs"](a))},initiateDirectMessages:function(a,b){var c=app.generatePublicKeyMessages(a),d=[],e;for(e in c)d.push({receiverId:a,text:c[e]});b||d.push({text:app.joinMessage,receiverId:a});app.postDirectMessages(d,function(){var b={};b[a]=!0;app.trackSentPublicKeys(b);app.checkDirectMessageStatus()})},
joinMessage:"I'm using Seecret to send encrypted direct messages over Twitter. Try it with me! Just go to https://www.seecret.net",prepareDirectMessage:function(a){var b=$("#directMessage").val();app.encryptDirectMessage(b,a)},decryptPrivateKey:function(a){var b=app.activeUser.keyOptions,c=null;app.passphrase&&(c=app.passphrase);b.passphrase&&(c=b.passphrase);b.passphrase||null!=app.passphrase||(app.passphrase=c=prompt("Enter your passphrase"));return a.decrypt(c)},encryptDirectMessage:function(a,
...
if(null!=a)return a=app.hideAndCompress(a.armored),app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH=1E4,chain=app.seecret_engine.chainify(a,["Here's my Seecret key","Now we can start using Seecret to send encrypted messages on Twitter."]),chain.reverse(),app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH=140,chain},promptDeleteKey:function(){confirm("Are you sure?")&&(app.deleteSavedItem("privateKey-"+app.activeUser.user.id_str),app.deleteSavedItem("publicKey-"+app.activeUser.user.id_str),app.deleteSavedItem("keyOptions-"+
app.activeUser.user.id_str),app.activeUser.privateKey=null,app.activeUser.publicKey=null,app.activeUser.keyOptions=null,app.renderPrivateKeySettings(),app.setView(app.SETTINGS_VIEW))},checkDirectMessageStatus:function(){app.doScroll=!1;var a=$("#selectedReceiverId").val(),b=$("#selectedReceiverName").val();if(a)if(null==app.activeUser.privateKey)app.showPrivateKeyManager();else{var c=app.activeUser.publicKeys,d=app.activeUser.sentPublicKeys;null==d||null==d[a]||null!=c&&c[a]?null==c||null!=c&&!c[a]?
app.showPublicKeyManager(a,b):app.showDirectMessageForm(a,b):app.showWaitingForPublicKeyView(a,b);app.doScroll=!0}},maxId:null,lastId:null,homeTimeline:!0,userTimelineId:null,updateUserTimeline:function(a,b){b&&(app.maxId=null);app.userTimelineId=a;app.homeTimeline=!1;app.updatingTimeline=!0;var c={data:{}};c.data.count=200;c.data.screen_name=a;c.data.include_rts=!1;null!=app.maxId?c.data.max_id=app.maxId:$("#status-list").empty();app.hideOverlays();app.getUserTimeline(c,app.handleTimelineResponse)},
getTimeline:function(a,b){app.oauthResult.get("1.1/statuses/home_timeline.json",a).done(function(a){b(a);app.hideOverlays()}).fail(app.handleGetTimelineError)},getUserTimeline:function(a,b){app.oauthResult.get("1.1/statuses/user_timeline.json",a).done(function(a){b(a);app.hideOverlays()}).fail(app.handleGetTimelineError)},handleTimelineResponse:function(a){0<a.length?1==a.length&&a[0].id_str==app.maxId?$("#status-list").append(Handlebars.templates["no-more-timeline-message-template.hbs"]()):app.processTimelineResponse(a):
...
a[d].seecret_envelope,f=app.seecret_engine.getSeecretFromEnvelope(e),e=app.seecret_engine.getContentTypeFromEnvelope(e),f=app.seecret_engine.unhide(f,e);if(e==app.seecret_engine.config.CONTENT_TYPES.NUMBERS_ARRAY)try{f=seecret_compression.decompress(f),a[d].seecret=f}catch(g){console.log("error decompressing a seecret message: "+JSON.stringify(g)),a[d].seecret="Could not decompress the Seecret",a[d].seecret_error=g}else a[d].seecret=f;c.push(a[d])}else b&&!app.seecret_engine.hasSeecretContent(a[d].text)&&
c.push(a[d]);return c},getEarliestIncompleteChainStartId:function(a,b){for(var c in b)app.getFirstIncompleteChainStart(a,b[c])},getEarliestIncompleteDMChainStartId:function(a,b){for(var c in b)app.getFirstIncompleteDMChainStart(a,b[c])},getFirstIncompleteChainStart:function(a,b){var c=0,d;for(d in a)a[d].user.id_str==b&&app.seecret_engine.hasSeecretContent(a[d].text)&&(app.seecret_engine.isEnvelopeStart(a[d].text)&&(c=d),app.seecret_engine.isEnvelopeEnd(a[d].text)&&(c=0));return c},getFirstIncompleteDMChainStart:function(a,
...
{data:{}};a.data.count=200;null!=app.maxId&&(a.data.max_id=app.maxId);return a},initTimeline:function(){app.maxId=null;app.homeTimeline=!0;app.userTimelineId=null;app.updateHomeTimeline()},updateHomeTimeline:function(a){a&&(app.maxId=null);app.homeTimeline=!0;$("#timelineName").empty();app.updatingTimeline=!0;a=app.generateTimelinePostVals();a.data.max_id||($("#status-list").empty(),app.lastProcessedStatusId=null);app.overlay();app.getTimeline(a,app.handleTimelineResponse)},postCompleteCount:0,postCompleteTarget:0,
prepareUpdateMessage:function(){var a=$("#textarea-statusUpdate").val(),a=app.hideAndCompress(a),b=null;app.useAutogeneratedScaffolding()||(b=app.getScaffolds());a=app.seecret_engine.chainify(a,b);a.reverse();return a},updateStatus:function(){var a=this.prepareUpdateMessage();this.postChain(a)},postChain:function(a){this.currentMessageChain=a;this.chainCount=0;this.postChainItem()},postChainItem:function(){this.chainCount<this.currentMessageChain.length?app.oauthResult.post("1.1/statuses/update.json",
{data:{status:this.currentMessageChain[this.chainCount++]}}).done(function(a){setTimeout(function(){app.postChainItem()},50)}).fail(function(a){}):($("#status-list").empty(),$("#scaffoldingWarningContainer").empty(),$("#scaffoldingStatus").empty(),$("#textarea-statusUpdate").val(""),$("#textarea-statusUpdate").val(""),$("#scaffoldsContainer").html(Handlebars.templates["scaffolds-template.hbs"]()),$("#scaffoldingContainer > div > div > input").val(""),app.maxId=null,this.currentMessageChain=[],this.chainCount=
...
$("#privateKeyPassphrase").val("");$("#privateKeyCopyarea").empty();$("#publicKeysDisplayArea").empty();app.unauthenticatedHeader();$("#privateMessageButton").hide();$("#timelineButton").hide();app.unauthenticatedMenu();app.setView(app.ABOUT_VIEW)},authenticatedMenu:function(){$("#privateMessageButton").show();$("#timelineButton").show();$("#settingsButton").show();$("#logoutButton").show();$("#hashesButton").show();$("#publicKeysButton").show();$("#aboutButton").show()},unauthenticatedMenu:function(){$("#privateMessageButton").hide();
$("#timelineButton").hide();$("#settingsButton").hide();$("#logoutButton").hide();$("#publicKeysButton").hide()},authenticatedHeader:function(){$("#loginContainer").html(Handlebars.templates["authenticated-header-template.hbs"](app.activeUser.user))},unauthenticatedHeader:function(){$("#loginContainer").html(Handlebars.templates["unauthenticated-header-template.hbs"]())},hideAllViews:function(){$("#timeline").hide();$("#statusMessage").hide();$("#privateMessages").hide();$("#settings").hide();$("#publicKeys").hide();
$("#about").hide();$("#hashes").hide()},setMenuState:function(a){$.each($(".menuLink"),function(a,c){$(this).css("background","");$(this).css("color","")});$("#"+a).css("background","#e3e3e3");$("#"+a).css("color","#333");app.openClosePanel("close")},setView:function(a){app.doScroll=!1;app.hideAllViews();$("#"+a).fadeIn(function(){app.doScroll=!0}).css("display","block");app.setMenuState(a+"Button")},renderPublicKeys:function(){var a=app.activeUser.publicKeys,b=[],c=0,d;for(d in a)a[d].displayDate=
moment(a[d].receiveDate).format("MMMM Do YYYY, h:mm:ss a"),b.push(a[d]),c++;0<c?(a=Handlebars.templates["public-keys-template.hbs"]({keys:b}),$("#publicKeysDisplayArea").html(a)):$("#publicKeysDisplayArea").html("No public keys")},copySenderPublicKeyToClipboard:function(a){var b=app.activeUser.publicKeys;b[a]&&(executeCopy(b[a].key),alert(b[a].screen_name+"'s key has been copied to your clipboard"))},copyContentsToClipboard:function(a){executeCopy($("#"+a).html())},showPrivateKeyManager:function(){app.doScroll=
!1;$("#directMessagesActionsContainer").html(Handlebars.templates["private-key-template.hbs"]());$("#directMessagesActionsContainer").show()},openClosePanel:function(a){"open"==a?$("#left-panel").css("left",0):$("#left-panel").css("left","-14em")},processScaffoldField:function(a){a=$("#scaffold"+a);val=a.val();for(var b=val.replace(/\s{2,}/g," ");" "==b.substr(0,1);)b=b.substr(1);b=b.substr(0,100);b!=val&&a.val(b)},useAutogeneratedScaffolding:function(){return!1},hideAndCompress:function(a){var b=
app.seecret_engine.hide(a,app.seecret_engine.config.CONTENT_TYPES.PLAIN),b=app.seecret_engine.envelope(b,app.seecret_engine.config.CONTENT_TYPES.PLAIN);a=seecret_compression.compress(a);a=app.seecret_engine.hide(a,app.seecret_engine.config.CONTENT_TYPES.NUMBERS_ARRAY);a=app.seecret_engine.envelope(a,app.seecret_engine.config.CONTENT_TYPES.NUMBERS_ARRAY);return b.length>a.length?a:b},processScaffolding:function(){var a=$("#textarea-statusUpdate").val();if(!app.useAutogeneratedScaffolding()){var b=
!1,a=app.hideAndCompress(a),c=app.getScaffolds(),d=0,e=0;if(1==c.length)if(2<=c[0].trim().length&&c[0].trim().length+a.length<app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH)b=!0,d=app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH-a.length-c[0].trim().length;else{if(2>c[0].trim().length){app.makeUpdateFormPending();return}e=app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH-(c[0].trim().length+a.length);e*=-1}else{var f=scaffoldLength=0;for(each in c)2<=c[each].trim().length&&(scaffoldLength+=
c[each].trim().length,f++);scaffoldLength+a.length<app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH*f?(b=!0,d=app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH*f-(scaffoldLength+a.length)):(e=app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH*f-(scaffoldLength+a.length),e*=-1)}b?app.makeUpdateFormReady(d):app.makeUpdateFormNotReady(e)}},newVisibleMessageField:function(){var a=app.getScaffolds();app.addScaffoldField(a.length);app.processScaffolding()},addScaffoldField:function(a,b){var c={scaffoldId:a,
val:b,scaffoldCount:a+1};$("#scaffoldsContainer").append(Handlebars.templates["scaffold-template.hbs"](c))},makeUpdateFormReady:function(a){$("#submitButtonContainer").show();var b={};b.availableCharacters=a;$("#scaffoldingStatus").html(Handlebars.templates["scaffold-status-good-template.hbs"](b))},makeUpdateFormNotReady:function(a){$("#submitButtonContainer").hide();var b={};b.spillover=a;$("#scaffoldingStatus").html(Handlebars.templates["scaffold-status-spillover-template.hbs"](b))},makeUpdateFormPending:function(){$("#submitButtonContainer").hide();
$("#scaffoldingStatus").html(Handlebars.templates["scaffold-status-incomplete-template.hbs"]())},removeScaffold:function(a){var b=app.getScaffolds(),c=[],d;for(d in b)d!=a&&c.push(b[d]);app.renderScaffolds(c);app.processScaffolding()},renderScaffolds:function(a){$("#scaffoldsContainer").html("");for(var b=0,c=0;c<a.length;c++)app.addScaffoldField(b++,a[c]);$("#scaffoldRemover0").remove()},getScaffolds:function(){for(var a=[],b=document.getElementById("scaffold0");b;)a.push(b.value.trim()),b=document.getElementById("scaffold"+
a.length);return a},toggleScaffolds:function(){document.getElementById("scaffoldingOption").checked?($("#scaffoldingStatus").html(""),$("#scaffoldsContainer").html(""),$("#scaffoldingWarningContainer").hide(),$("#scaffoldingContainer").hide(),$("#submitButtonContainer").show()):($("#scaffoldingStatus").html(""),$("#scaffoldsContainer").html(""),$("#scaffoldingContainer").show(),$("#scaffoldingWarningContainer").html(""),$("#submitButtonContainer").hide(),app.addScaffoldField(0),app.processScaffolding())},
getScaffoldTotal:function(a){for(var b=0,c=0;c<a.length;c++)b+=a[c].trim().length;return b},showCopyPrivateKey:function(a){"block"==$("#privateKeyCopyarea").css("display")?($(a).html("Show"),$("#privateKeyCopyarea").hide(),$("#privateKeyCopyarea").html("")):($(a).html("Hide"),app.renderPrivateKeySettings())},toggleDeleteSettings:function(a){"block"==$("#deleteKeyForm").css("display")?($(a).html("Show"),$("#deleteKeyForm").hide()):($(a).html("Hide"),$("#deleteKeyForm").show())},renderPrivateKeySettings:function(){var a=
app.activeUser.publicKey,b=app.activeUser.keyOptions,c={};c.privateKey=app.activeUser.privateKey;c.privateKey.dateCreatedDisplay=(new moment(c.privateKey.dateCreated)).format("MMMM Do YYYY, h:mm:ss a");c.publicKey=a;c.keyOptions=b;a=Handlebars.templates["user-keys-template.hbs"](c);$("#privateKeyCopyarea").html(a);$("#privateKeyCopyarea").fadeIn()},showPublicKeys:function(a){"block"==$("#publicKeysDisplayArea").css("display")?($(a).html("Show"),$("#publicKeysDisplayArea").hide()):($(a).html("Hide"),
app.renderPublicKeys(),$("#publicKeysDisplayArea").fadeIn())},showImportPrivateKey:function(a){"block"==$("#importForm").css("display")?($(a).html("Show"),$("#importForm").hide()):($(a).html("Hide"),$("#importForm").show())},showGeneratePrivateKey:function(a){"block"==$("#generateForm").css("display")?($(a).html("Show"),$("#generateForm").hide()):($(a).html("Hide"),$("#generateForm").show())},saveObject:function(a,b){localStorage[a]=JSON.stringify(b)},getObject:function(a){return app.getLocalObject(a)},
...
app.renderPrivateKeySettings()},privateKeyTest:function(){var a=openpgp.key.readArmored(app.activeUser.privateKey.armored).keys[0];alert(app.decryptPrivateKey(a))},overlay:function(a){a||(a="overlay");$("#"+a).css("display","block")},hideOverlays:function(){$("#overlay").css("display","none");$("#static-overlay").css("display","none")},getNameValuePairsFromArgumentList:function(a,b,c){var d=[];a=a.split(b);for(var e in a)b=a[e].split(c),d.push({name:b[0],value:b[1]});return d}};
