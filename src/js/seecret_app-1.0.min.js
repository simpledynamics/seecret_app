...
c);app.activeUser.publicKeys=c}},processDirectMessagesResponse:function(a){1==a.length&&a[0].id_str==app.dmMaxId?($("#directMessages").append(Handlebars.templates["no-more-direct-messages-template.hbs"]()),app.hideOverlays(),app.updatingDirectMessages=!1):0==a.lenth&&app.dmMaxId?($("#directMessages").append(Handlebars.templates["no-seecrets-in-direct-messages-template.hbs"]()),app.hideOverlays(),app.updatingDirectMessages=!1):(a=app.processDirectMessages(a,app.getUniqueDMSenders),app.timelineContainsKeys(a)&&
...
(b={});for(var c in a)b[c]=new Date;app.activeUser.sentPublicKeys=b;app.saveObject("sentPublicKeys-"+app.activeUser.user.id_str,b)},passphrase:null,decryptDirectMessages:function(a){var b=openpgp.key.readArmored(app.activeUser.privateKey.armored).keys[0];app.decryptPrivateKey(b)?app.verifyMessagesDecrypted(a,b):app.updateDirectMessagesUI(a)},verifyMessagesDecrypted:function(a,b){var c=null,d;for(d in a)if(a[d].encrypted&&0==a[d].decrypted){c=d;break}var e=app.activeUser.user;if(null!=c)if(a[c].sender.id_str==
e.id)a[c].decrypted=!0,app.verifyMessagesDecrypted(a,b);else{a[d].decrypted=!0;a[d].decryptionSuccessful=!1;a[d].originaltext=a[d].text;try{var f=openpgp.message.readArmored(a[c].seecret),g={privateKey:b,message:f};if(app.activeUser.publicKeys&&app.activeUser.publicKeys[a[c].sender.id_str]){var l=openpgp.key.readArmored(app.activeUser.publicKeys[a[c].sender.id_str].key);g.publicKeys=l.keys}try{openpgp.decrypt(g).then(function(b){a[d].decryptionSuccessful=!0;a[d].seecret=b.data;b.signatures&&0<b.signatures.length&&
b.signatures[0].valid&&(a[d].signed=!0);app.verifyMessagesDecrypted(a,g.privateKey)},function(b){a[d].text=app.generateUndecryptedMessageContent(a[d]);app.verifyMessagesDecrypted(a,g.privateKey)})}catch(k){console.log("try/catch error decrypting: "+JSON.stringify(k)),a[d].text=app.generateUndecryptedMessageContent(a[d]),app.verifyMessagesDecrypted(a,b)}}catch(k){console.log("major try/catch error decrypting: "+JSON.stringify(k)),a[d].text=app.generateUndecryptedMessageContent(a[d]),app.verifyMessagesDecrypted(a,
b)}}else{(c=app.activeUser.dmSenders)||(c={});for(var h in a)a[h].encrypted&&a[h].sender.id_str!=app.activeUser.id&&(c[a[h].sender.id_str]=!0);app.saveObject("dmSenders-"+app.activeUser.user.id_str,c);app.refreshActiveUserDMSenders();app.updateDirectMessagesUI(a)}},undecryptedMessageContentHandler:null,generateUndecryptedMessageContent:function(a){return Handlebars.templates["undecrypted-message-content-template.hbs"](a)},showPublicKeyManager:function(a,b){app.doScroll=!1;var c={};c.receiver_name=
b;c.receiver_id=a;$("#directMessagesActionsContainer").html(Handlebars.templates["public-key-template.hbs"](c))},showWaitingForPublicKeyView:function(a,b){app.doScroll=!1;var c={};c.receiver_name=b;c.receiver_id=a;c.invite_date=moment(app.activeUser.sentPublicKeys[a]).format("MMMM Do YYYY, h:mm:ss a");$("#directMessagesActionsContainer").html(Handlebars.templates["public-key-pending-template.hbs"](c))},showDirectMessageForm:function(a,b){app.doScroll=!1;var c={receiverId:a,receiverName:b};$("#directMessagesActionsContainer").html(Handlebars.templates["direct-message-template.hbs"](c));
app.doScroll=!1},makeKeyOptionsForUser:function(a){var b=app.generatePassword();return{numBits:1024,userIds:[{name:a.name,email:a.screen_name+"@seecretApp.seecret"}],passphrase:b}},generatePassword:function(){for(var a="",b=0;50>b;b++){var c=Math.floor(96*Math.random())+31;92==c||34==c||39==c||36==c?b--:a+=String.fromCharCode(c)}return a},validateNewPrivateKey:function(a){if(1>$("#"+a).val().length)return $("#"+a+"Error").html("You must have a passphrase for your key of at least one character"),!1;
$("#"+a+"Error").html("");if(confirm("Are you sure?")){var b=$("#"+a).val(),c=1==document.getElementById(a+"Cache").checked;app.hideAllViews();$("#"+app.STATUS_MESSAGE_VIEW).show();app.generatePrivateKeyPair(b,c,function(){$("#"+a).val("");app.setView(app.DIRECT_MESSAGES_VIEW);confirm("Send your new key to the contacts who have the old one?")&&app.resendPublicKeys();$("#directMessagesActionsContainer").empty()})}},resendPublicKeys:function(){var a=app.activeUser.sentPublicKeys,b=app.activeUser.dmSenders;
if(null!=a||null!=b){var c=[],d={},e;for(e in a)d[e]=!0;for(e in b)d[e]=!0;var a=0,f;for(f in d){a++;var b=app.generatePublicKeyMessages(f),g;for(g in b)c.push({receiverId:f,text:b[g]})}0<c.length&&app.sendPublicKeyMessages(c,d)}},generatePrivateKeyPair:function(a,b,c){var d=app.activeUser.user,e=app.makeKeyOptionsForUser(d);e.passphrase=a;d.id_str||(e.userId=d.id_str);null!=d&&(a=new Date,console.log("start generating key pair : "+a.getSeconds()+":"+a.getMilliseconds()),openpgp.generateKey(e).then(function(a){app.generatePrivateKeyPairSuccess(a,
e,b,c)}))},generatePrivateKeyPairSuccess:function(a,b,c,d){var e=new Date;console.log("done generating key pair: "+e.getSeconds()+":"+e.getMilliseconds());c||delete b.passphrase;c=app.activeUser.user;var e=new Date,f={};f.armored=a.privateKeyArmored;f.dateCreated=e;var g={};g.armored=a.publicKeyArmored;g.dateCreated=e;app.saveObject("keyOptions-"+c.id_str,b);app.saveObject("privateKey-"+c.id_str,f);app.saveObject("publicKey-"+c.id_str,g);app.checkDirectMessageStatus();app.refreshActiveUserTotal();
d()},setDirectMessageReceiver:function(a,b){$("#selectedReceiverId").val(a);$("#selectedReceiverName").val(b);var c={};c.name=b;c.id_str=a;var d=app.activeUser.publicKeys,e;for(e in d)d[e].id_str==a&&(c.publicKey=d[e],c.publicKey.displayDate=(new moment(c.publicKey.receiveDate)).format("MMMM Do YYYY, h:mm:ss a"));$("#friendName").html(Handlebars.templates["direct-message-friend-name.hbs"](c));app.checkDirectMessageStatus();$(".dmMessageDiv").hide();$(".dm_"+a).fadeIn()},updateDirectMesssagesBreadcrumbs:function(a){$("#directMessagesBreadCrumbContainer").html(Handlebars.templates["direct-message-breadcrumb-template.hbs"](a))},
