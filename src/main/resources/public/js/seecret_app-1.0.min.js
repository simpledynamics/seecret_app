...
a)?b.screen_name:b,a))+"</a>  "+c.escapeExpression((e.tweetShortDate||a&&a.tweetShortDate||e.helperMissing).call(a,null!=a?a.created_at:a,{name:"tweetShortDate",hash:{},data:d}))+"  \r\n"+(null!=(b=e["if"].call(a,null!=a?a.decryptionSuccessful:a,{name:"if",hash:{},fn:c.program(2,d,0),inverse:c.noop,data:d}))?b:"")+(null!=(b=e["if"].call(a,null!=a?a.signed:a,{name:"if",hash:{},fn:c.program(4,d,0),inverse:c.noop,data:d}))?b:"")+(null!=(b=e["if"].call(a,null!=a?a["new"]:a,{name:"if",hash:{},fn:c.program(6,
d,0),inverse:c.noop,data:d}))?b:"")+'\t\t\t\t\t\t\t\t</span>\r\n\t\t\t\t\t\t\t\t<div class="directMessage">'+(null!=(b=e["if"].call(a,null!=a?a.decryptionSuccessful:a,{name:"if",hash:{},fn:c.program(8,d,0),inverse:c.program(10,d,0),data:d}))?b:"")+"</div>\r\n\t\t\t\t\t\t\t</div>\r\n\t\t\t\t\t\t</div>\r\n\t\t\t\t\t</div>\r\n"},2:function(c,a,e,f,d){return'\t\t\t\t\t\t\t\t<img src="img/encrypted.png" class="dmIcon" />\r\n'},4:function(c,a,e,f,d){return'\t\t\t\t\t\t\t\t<img src="img/signed.jpg" class="dmIcon" />\r\n'},
6:function(c,a,e,f,d){return'\t\t\t\t\t\t\t\t<img src="img/new-icon.png" class="dmIconNew" />\r\n'},8:function(c,a,e,f,d){var b;return"\r\n\t\t\t\t\t\t\t\t"+c.escapeExpression((b=null!=(b=e.seecret||(null!=a?a.seecret:a))?b:e.helperMissing,"function"===typeof b?b.call(a,{name:"seecret",hash:{},data:d}):b))+"\r\n"},10:function(c,a,e,f,d){var b,g;return"\t\t\t\t\t\t\t\t"+(null!=(b=(g=null!=(g=e.text||(null!=a?a.text:a))?g:e.helperMissing,"function"===typeof g?g.call(a,{name:"text",hash:{},data:d}):
g))?b:"")+"\r\n\t\t\t\t\t\t\t\t"},useData:!0})})();
...
setupDMPostData:function(){app.dmPostData={count:app.NUM_DM,full_text:!0,include_entities:!1,skip_status:!0}},startDirectMessages:function(){app.setupDMPostData();app.overlay();app.doDMPost(app.dmPostData,app.handleDMResponse)},doDMPost:function(a,b){app.updatingDirectMessages=!0;app.timer("direct message post");app.oauthResult.get("1.1/direct_messages.json",{data:a}).done(function(a){app.timer("direct message post");0<a.length?(a.length==app.NUM_DM&&$("#getOlderMessagesButton").show(),app.dmSinceId=
a[0].id_str,b(a)):(app.hideOverlays(),app.dmPostData.since_id?alert("No new direct messages"):app.dmPostData.max_id||$("#directMessages").html("You have no direct messages."))}).fail(function(a){app.hideOverlays()})},handleDMResponse:function(a){app.timer("handleDMResponse");var b=null!=app.dmPostData.since_id;if(b&&0==a.length)alert("No new messages");else{$(".dmIconNew").hide();console.log("handling dm response for "+a.length);if(1==a.length&&a[0].id_str==app.dmMaxId)app.hasFirstDirectMessage||
($("#directMessages").append(Handlebars.getTemplate("no-more-direct-messages-template")()),$("#getOlderMessagesButton").hide()),app.hasFirstDirectMessage=!0,app.hideOverlays(),app.updatingDirectMessages=!1;else if(0==a.length)$("#directMessages").append(Handlebars.getTemplate("no-seecrets-in-direct-messages-template")()),app.hideOverlays(),app.updatingDirectMessages=!1;else{var c=app.processMessageList(a,"sender","dmMaxId");app.timelineContainsKeys(c)&&(console.log("saving public keys!"),app.savePublicKeys(app.getPublicKeyMessagesFromDMList(c)));
var d=app.getFriendsFromMessageList(c);app.markMessageableFriends(d);app.markFriendsWithSentKeys(d);console.log("Found "+d.length+" friends");if($("#friendsContainer").is(":empty"))$("#friendsContainer").html(Handlebars.getTemplate("friends-list-template")(d));else{var e=[],g;for(g in d)null==document.getElementById("friendDetails_"+d[g].id_str)&&e.push(d[g]);0<e.length?$("#friendsContainer").prepend(Handlebars.getTemplate("friends-list-template")(e)):console.log("no new friends")}if(b){b="New messages from ";
e="";for(g in d)b+=e+d[g].screen_name,e=",";0<a.length&&alert(b)}app.timelineContainsEncryptedMessages(c)?(app.markEncryptedDirectMessages(c),app.decryptDirectMessages(c)):app.updateDirectMessagesUI(c,d)}app.hideOverlays();app.timer("handleDMResponse")}},updateDirectMessagesUI:function(a,b){app.hideOverlays();b=b?b:app.getFriendsFromMessageList(a);app.renderAllFriendMessages(b,a);app.updatingDirectMessages=!1},markMessageableFriends:function(a){var b=app.getObject("publicKeys-"+app.activeUser.user.id_str),
c;for(c in a)console.log("the public key is "+b[a[c].id_str]),a[c].publicKey=b[a[c].id_str]},markFriendsWithSentKeys:function(a){console.log("marking friends with sent keys!");var b=app.getObject("sentPublicKeys-"+app.activeUser.user.id_str),c;for(c in b)for(var d in a)c==a[d].id_str&&(a[d].sentKey=b[c])},renderAllFriendMessages:function(a,b){for(var c in a)app.renderFriendMessages(a[c].id_str,b)},renderFriendMessages:function(a,b){var c=app.filterMessageListByFriendId(b,a);if(app.dmPostData.since_id){for(var d in c)c[d].new=
!0;$("#friendMessages_"+a).prepend(Handlebars.getTemplate("direct-messages-template")(c))}else $("#friendMessages_"+a).append(Handlebars.getTemplate("direct-messages-template")(c))},getFriendsFromMessageList:function(a){var b={},c;for(c in a)b[a[c].sender.screen_name]||(b[a[c].sender.screen_name]=a[c].sender,b[a[c].sender.screen_name].most_recent=a[c],console.log("Most recent message from "+a[c].sender.screen_name+" is "+a[c].created_at)),b[a[c].sender.screen_name].message_count||(b[a[c].sender.screen_name].message_count=
0),b[a[c].sender.screen_name].message_count++;a=[];for(f in b)a.push(b[f]);return a},toggleFriendMessages:function(a){"show conversation"!=$("#friendMessagesToggleLink_"+a).html()?($("#friendMessagesToggleLink_"+a).html("show conversation"),$("#friendMessages_"+a).fadeOut()):($("#friendMessagesToggleLink_"+a).html("hide"),$("#friendMessages_"+a).fadeIn())},filterMessageListByFriendId:function(a,b){var c=[],d;for(d in a)a[d].sender.id_str!=b&&a[d].sender.id_str!=app.activeUser.id_str||c.push(a[d]);
console.log("filtered "+a.length+" down to "+c.length);return c},getOlderDirectMessages:function(){app.hasFirstDirectMessage||(app.setupDMPostData(),app.dmPostData.max_id=app.dmMaxId,app.doDMPost(app.dmPostData,function(a){if(a.length<app.NUM_DM||app.dmMaxId==a[a.length-1].id_str)app.hasFirstDirectMessage=!0,$("#directMessages").append(Handlebars.getTemplate("no-more-direct-messages-template")()),$("#getOlderMessagesButton").hide();app.handleDMResponse(a)}))},isPublicKey:function(a){return app.publicKeyStartRegex.test(a)&&
...
e;var h={};h.armored=a.publicKeyArmored;h.dateCreated=e;app.saveObject("keyOptions-"+c.id_str,b);app.saveObject("privateKey-"+c.id_str,g);app.saveObject("publicKey-"+c.id_str,h);app.checkDirectMessageStatus();app.refreshActiveUserTotal();d()},showAllDirectMessages:function(){$(".friendMessages").show()},initiateDirectMessages:function(a,b){var c=app.generatePublicKeyMessages(a),d=[],e;for(e in c)d.push({receiverId:a,text:c[e]});b||d.push({text:app.joinMessage,receiverId:a});app.postDirectMessages(d,
function(){var b={};b[a]=!0;app.trackSentPublicKeys(b)})},prepareDirectMessage:function(a){if(!app.sendingDirectMessage){var b=$("#directMessage_"+a).val();0!=b.length&&($("#directMessage_"+a).val(""),app.sendingDirectMessage=!0,app.overlay(),app.encryptDirectMessage(b,a))}},decryptPrivateKey:function(a){var b=app.activeUser.keyOptions,c=null;app.passphrase&&(c=app.passphrase);b.passphrase&&(c=b.passphrase);b.passphrase||null!=app.passphrase||(app.passphrase=c=prompt("Enter your passphrase"));if(a.decrypt(c))return!0;
app.passphrase=null;return!1},encryptDirectMessage:function(a,b){var c=app.activeUser.publicKeys,d=null,e;for(e in c)e==b&&(d=c[e].key);d?(c=openpgp.key.readArmored(d).keys,c={publicKeys:c,data:a},app.activeUser.privateKey&&(d=openpgp.key.readArmored(app.activeUser.privateKey.armored),app.decryptPrivateKey(d.keys[0])&&(c.privateKeys=d.keys)),openpgp.encrypt(c).then(function(a){a=app.hideAndCompress(a.data);app.seecret_engine.config.RANDOM_COVERTEXTS=!0;app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH=
1E4;a=app.seecret_engine.chainify(a,covertexts);app.seecret_engine.config.RANDOM_COVERTEXTS=!1;app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH=140;a.reverse();for(var c in a)a[c]={receiverId:b,text:a[c]};app.postDirectMessages(a,function(){});$("#directMessage").val("")}).catch(function(a){console.log("Error generating the messages:"+JSON.stringify(a));app.hideOverlays();app.sendingDirectMessage=!1})):(app.sendingDirectMessage=!1,app.hideOverlays(),alert("You do not have that users public key."))},
postDirectMessages:function(a,b){app.directMessagesToPost=a;app.directMessageCount=0;app.checkDirectMessages(b)},checkDirectMessages:function(a){if(app.directMessagesToPost&&app.directMessageCount<app.directMessagesToPost.length){var b=app.directMessagesToPost[app.directMessageCount].text,c=app.directMessagesToPost[app.directMessageCount].receiverId;app.directMessageCount++;app.postDirectMessage(b,c,a)}else app.hideOverlays(),alert("Sent!"),app.sendingDirectMessage=!1,a&&a()},postDirectMessage:function(a,
...
$("#importPrivateKeyText").val("");$("#importPrivateKeyPassphrase").val("");$("#privateKeyTextarea").val("");$("#privateKeyPassphrase").val("");$("#privateKeyCopyarea").empty();$("#publicKeysDisplayArea").empty();app.unauthenticatedHeader();$("#privateMessageButton").hide();$("#timelineButton").hide();app.unauthenticatedMenu();app.setView(app.ABOUT_VIEW)},authenticatedMenu:function(){$("#privateMessageButton").show();$("#timelineButton").show();$("#settingsButton").show();$("#logoutButton").show();
$("#hashesButton").show();$("#publicKeysButton").show();$("#aboutButton").show()},unauthenticatedMenu:function(){$("#privateMessageButton").hide();$("#timelineButton").hide();$("#settingsButton").hide();$("#logoutButton").hide();$("#publicKeysButton").hide()},authenticatedHeader:function(){app.activeUser&&app.activeUser.user&&(app.activeUser.user.profile_image_url=app.activeUser.user.profile_image_url.replace(app.httpsReplaceRegex,"https:"),$("#loginContainer").html(Handlebars.getTemplate("authenticated-header-template")(app.activeUser.user)))},
...
$(document).ready(function(){openpgp.config.show_comment=!1;openpgp.config.show_version=!1;app.seecret_engine=new SEECRET_ENGINE;$(window).scroll(function(){if(app.doScroll){var a=$(window).scrollTop(),b=$(window).height(),c=$(document).height();a+b==c&&("block"!=document.getElementById("timeline").style.display||app.updatingTimeline?document.getElementById("privateMessages"):app.homeTimeline?app.updateHomeTimeline():app.updateUserTimeline(app.userTimelineId))}});$("#textarea-statusUpdate").keyup(function(a){app.processScaffolding()});
$("#textarea-statusUpdate").change(function(a){app.processScaffolding()});$("#scaffoldingEntryContainer0").keyup(function(a){app.processScaffoldField(0);app.processScaffolding()});$("#updateStatusForm").submit(function(a){a.preventDefault();app.updateStatus()});$.ajax({url:"hashes.txt"}).done(function(a){a=app.getNameValuePairsFromArgumentList(a,/\n/,"=");if(null==a)$("#hashesContent").html(Handlebars.getTemplate("hashes-not-found-template")()),app.setView(app.HASHES_VIEW),app.unauthenticatedMenu();
