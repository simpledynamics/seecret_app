...
app.activeUser.user.id_str);b&&(a.publicKey=b[a.id_str]);if(b=app.getObject("sentPublicKeys-"+app.activeUser.user.id_str))a.sentKey=b[a.id_str];$("#friendsContainer").append(Handlebars.getTemplate("conversation-template")(a))},handleDMResponse:function(a){app.timer("handleDMResponse");for(var b in a);1==a.length&&a[0].id_str==app.dmMaxId?(app.hasFirstDirectMessage=!0,app.updatingDirectMessages=!1):0==a.length?($("#directMessages").append(Handlebars.getTemplate("no-seecrets-in-direct-messages-template")()),
app.hideOverlays(),app.updatingDirectMessages=!1):(a=app.processMessageList(a,"sender","dmMaxId"),app.timelineContainsKeys(a)&&app.savePublicKeys(app.getPublicKeyMessagesFromDMList(a)),app.timelineContainsEncryptedMessages(a)?(app.markEncryptedDirectMessages(a),app.decryptDirectMessages(a)):(app.createConversationEntries(a),app.updateDirectMessagesUI(a,friendList)));app.hideOverlays();app.timer("handleDMResponse")},createConversationEntries:function(a){var b=app.getFriendsFromMessageList(a);if(null!=
app.dmPostData.since_id){var c="New messages from ",d="",e;for(e in b)c+=d+b[e].screen_name,d=",";0<a.length&&alert(c);for(e in b)b[e].newMessages=!0,b[e].newMessageCount=8<b[e].message_count?9:b[e].message_count}app.markMessageableFriends(b);app.markFriendsWithSentKeys(b);app.markFriendsWithUnencryptedMessages(b,a);if($("#friendsContainer").is(":empty"))$("#friendsContainer").html(Handlebars.getTemplate("conversations-template")(b));else{a=[];for(e in b)if(null==document.getElementById("friendDetails_"+
...
$("#timelineButton").hide();$("#settingsButton").hide();$("#logoutButton").hide();$("#publicKeysButton").hide()},authenticatedHeader:function(){app.activeUser&&app.activeUser.user&&(console.log("app.active user = "+JSON.stringify(app.activeUser.user)),app.activeUser.user.profile_image_url=app.activeUser.user.profile_image_url.replace(app.httpsReplaceRegex,"https:"),$("#loginContainer").html(Handlebars.getTemplate("authenticated-header-template")(app.activeUser.user)))},unauthenticatedHeader:function(){$("#loginContainer").html(Handlebars.getTemplate("unauthenticated-header-template")())},
