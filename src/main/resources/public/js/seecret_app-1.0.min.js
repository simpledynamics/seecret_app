...
app.activeUser.user.id_str),app.deleteSavedItem("keyOptions-"+app.activeUser.user.id_str),app.activeUser.privateKey=null,app.activeUser.publicKey=null,app.activeUser.keyOptions=null,app.renderPrivateKeySettings(),app.setView(app.SETTINGS_VIEW))},openDirectMessageForm:function(a){$("#friendMessageForm_"+a).fadeIn()},checkDirectMessageStatus:function(){app.doScroll=!1;var a=$("#selectedReceiverId").val(),b=$("#selectedReceiverName").val();if(a)if(null==app.activeUser.privateKey)app.showPrivateKeyManager();
else{var c=app.activeUser.publicKeys,d=app.activeUser.sentPublicKeys;null==d||null==d[a]||null!=c&&c[a]?null==c||null!=c&&!c[a]?app.showPublicKeyManager(a,b):app.showDirectMessageForm(a,b):app.showWaitingForPublicKeyView(a,b);app.doScroll=!0}},updateUserTimeline:function(a,b){b&&(app.maxId=null);app.userTimelineId=a;app.homeTimeline=!1;app.updatingTimeline=!0;var c={data:{}};c.data.count=200;c.data.screen_name=a;c.data.include_rts=!1;null!=app.maxId?c.data.max_id=app.maxId:$("#status-list").empty();
app.hideOverlays();app.getUserTimeline(c,app.handleTimelineResponse)},getTimeline:function(a,b){app.oauthResult.get("1.1/statuses/home_timeline.json",a).done(function(a){b(a);app.hideOverlays()}).fail(app.handleGetTimelineError)},getUserTimeline:function(a,b){$("#twitterUser").val("");app.oauthResult.get("1.1/statuses/user_timeline.json",a).done(function(a){b(a);app.hideOverlays()}).fail(app.handleGetTimelineError)},handleTimelineResponse:function(a){app.timer("Get timeline");0<a.length?1==a.length&&
a[0].id_str==app.maxId?($("#status-list").append(Handlebars.getTemplate("no-more-timeline-message-template")()),$("#getOlderTimelineButton").hide()):(app.timer("Process timeline Response"),app.processTimelineResponse(a),app.timer("Process timeline Response")):app.handleEmptyTimelineResponse(a)},handleGetTimelineError:function(a){429==a.status?alert("Twitter rate limit exceeded.  You must wait 15 minutes to make more requests from this app."):404==a.status?alert("Twitter says that screen name does not exist."):
alert("There was an error retrieving the timeline.");console.log("Error:"+JSON.stringify(a));app.hideOverlays()},handleEmptyTimelineResponse:function(a){app.hideOverlays()},processTimelineResponse:function(a){app.addFollowerTagToTweets(a,app.processTimelineWithFollowerInfo)},addFollowerTagToTweets:function(a,b){var c={},d;for(d in a)c[a[d].user.id_str]=a[d].user.id_str;d=[];for(var e in c)d.push(e);c={data:{}};c.data.user_id=d.join();app.oauthResult.get("1.1/friendships/lookup.json",c).done(function(c){var d=
...
if(app.decryptPrivateKey(d.keys[0])){d.keys[0].primaryKey.encrypt(a);d.keys[0].subKeys&&0<d.keys[0].subKeys.length&&d.keys[0].subKeys[0]&&d.keys[0].subKeys[0].subKey.encrypt(a);var e={};e.dateCreated=new Date;e.armored=d.keys[0].armor();var g={};g.dateCreated=new Date;g.armored=d.keys[0].toPublic().armor();c.passphrase=a;app.saveObject("publicKey-"+b.id_str,g);app.saveObject("privateKey-"+b.id_str,e);app.saveObject("keyOptions-"+b.id_str,c);app.activeUser.publicKey=g;app.activeUser.privateKey=e;app.activeUser.keyOptions=
...
timer:function(a){var b=moment();app.timers[a]?(console.log(a+" took "+b.diff(app.timers[a])+" milliseconds"),delete app.timers[a]):app.timers[a]=b}};
