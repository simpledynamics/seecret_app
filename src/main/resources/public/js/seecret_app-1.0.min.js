...
var b=app.getObject("sentPublicKeys-"+app.activeUser.user.id_str),c;for(c in b)for(var d in a)c==a[d].id_str&&(console.log("found a sent key to "+c),a[d].sentKey=b[c])},renderAllFriendMessages:function(a,b){for(var c in a)app.renderFriendMessages(a[c].id_str,b)},renderFriendMessages:function(a,b){var c=app.filterMessageListByFriendId(b,a);app.dmPostData.sinceId?$("#friendMessages_"+a).prepend(Handlebars.getTemplate("direct-messages-template")(c)):$("#friendMessages_"+a).append(Handlebars.getTemplate("direct-messages-template")(c))},
getFriendsFromMessageList:function(a){var b={},c;for(c in a)b[a[c].sender.screen_name]||(b[a[c].sender.screen_name]=a[c].sender,b[a[c].sender.screen_name].most_recent=a[c],console.log("Most recent message from "+a[c].sender.screen_name+" is "+a[c].created_at)),b[a[c].sender.screen_name].message_count||(b[a[c].sender.screen_name].message_count=0),b[a[c].sender.screen_name].message_count++;a=[];for(f in b)a.push(b[f]);return a},toggleFriendMessages:function(a){"show conversation"!=$("#friendMessagesToggleLink_"+
a).html()?($("#friendMessagesToggleLink_"+a).html("show conversation"),$("#friendMessages_"+a).fadeOut()):($("#friendMessagesToggleLink_"+a).html("hide"),$("#friendMessages_"+a).fadeIn())},filterMessageListByFriendId:function(a,b){var c=[],d;for(d in a)a[d].sender.id_str!=b&&a[d].sender.id_str!=app.activeUser.id_str||c.push(a[d]);console.log("filtered "+a.length+" down to "+c.length);return c},getOlderDirectMessages:function(){app.hasFirstDirectMessage||(app.setupDMPostData(),app.dmPostData.max_id=
app.dmMaxId,app.doDMPost(app.dmPostData,function(a){if(a.length<app.NUM_DM||app.dmMaxId==a[a.length-1].id_str)app.hasFirstDirectMessage=!0,$("#directMessages").append(Handlebars.getTemplate("no-more-direct-messages-template")()),$("#getOlderMessagesButton").hide();app.handleDMResponse(a)}))},isPublicKey:function(a){return app.publicKeyStartRegex.test(a)&&app.publicKeyEndRegex.test(a)},isPGPMessage:function(a){return app.pgpMessageStartRegex.test(a)&&app.pgpMessageEndRegex.test(a)},timelineContainsKeys:function(a){for(var b in a)if(app.isPublicKey(a[b].seecret))return!0;
