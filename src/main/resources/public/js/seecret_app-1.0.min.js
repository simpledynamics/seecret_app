...
hash:{},data:f}):b))+'">show conversation</a> \r\n\t\tMost recent: '+e.escapeExpression((d.tweetShortDate||a&&a.tweetShortDate||d.helperMissing).call(a,null!=a?a.created_at:a,{name:"tweetShortDate",hash:{},data:f}))+"  \r\n"+(null!=(c=d["if"].call(a,null!=a?a.publicKey:a,{name:"if",hash:{},fn:e.program(2,f,0),inverse:e.program(4,f,0),data:f}))?c:"")+'\t\t<br/>\r\n\t\t<div id="friendMessages_'+e.escapeExpression((b=null!=(b=d.id_str||(null!=a?a.id_str:a))?b:d.helperMissing,"function"===typeof b?b.call(a,
{name:"id_str",hash:{},data:f}):b))+'" style="display:none" class="friendMessages"></div>\r\n\t</div>\r\n'},2:function(e,a,d,g,f){var c;return'\t\t\t<br/>\r\n\t\t\t<textarea id="directMessage_'+e.escapeExpression((c=null!=(c=d.id_str||(null!=a?a.id_str:a))?c:d.helperMissing,"function"===typeof c?c.call(a,{name:"id_str",hash:{},data:f}):c))+'" style="width:50%"></textarea>\r\n\t\t\t<input type=button onClick="app.prepareDirectMessage(\''+e.escapeExpression((c=null!=(c=d.id_str||(null!=a?a.id_str:a))?
c:d.helperMissing,"function"===typeof c?c.call(a,{name:"id_str",hash:{},data:f}):c))+'\')" value="Send message" class="button"/>\r\n'},4:function(e,a,d,g,f){var c;return"\t\t<br/>Not sure if "+e.escapeExpression((c=null!=(c=d.screen_name||(null!=a?a.screen_name:a))?c:d.helperMissing,"function"===typeof c?c.call(a,{name:"screen_name",hash:{},data:f}):c))+' has your public key?  <input type=button onClick="app.initiateDirectMessages('+e.escapeExpression((c=null!=(c=d.id_str||(null!=a?a.id_str:a))?c:
d.helperMissing,"function"===typeof c?c.call(a,{name:"id_str",hash:{},data:f}):c))+',true)" value="send it"/>  ('+e.escapeExpression((c=null!=(c=d.screen_name||(null!=a?a.screen_name:a))?c:d.helperMissing,"function"===typeof c?c.call(a,{name:"screen_name",hash:{},data:f}):c))+" can't send you encryted messages until you have sent them your key)\r\n"},useData:!0})})();
...
app.oauthResult?(app.authenticatedUI(),app.maxId=null,app.updateHomeTimeline()):app.unauthenticatedUI()},dmPostData:{},dmMaxId:null,dmSinceId:null,initializeDirectMessagesView:function(){app.setView(app.DIRECT_MESSAGES_VIEW);""==$("#directMessages").html()?app.startDirectMessages():app.getNewestDirectMessages()},getNewestDirectMessages:function(){app.setupDMPostData();app.dmSinceId?(app.dmPostData.since_id=app.dmSinceId,app.overlay(),app.doDMPost(app.dmPostData,app.handleDMResponse)):console.log("getting newest but there's no since id?  ")},
setupDMPostData:function(){app.dmPostData={count:100,full_text:!0,include_entities:!1,skip_status:!0}},startDirectMessages:function(){app.setupDMPostData();app.overlay();app.doDMPost(app.dmPostData,app.handleDMResponse)},doDMPost:function(a,b){app.updatingDirectMessages=!0;app.oauthResult.get("1.1/direct_messages.json",{data:a}).done(function(a){0<a.length?($("#olderDirectMessagesButtonContainer").show(),app.dmSinceId||(app.dmSinceId=a[0].id_str),b(a)):(app.hideOverlays(),app.dmPostData.since_id?
alert("No new direct messages"):app.dmPostData.max_id||$("#directMessages").html("You have no direct messages."))}).fail(function(a){app.hideOverlays()})},handleDMResponse:function(a){var b=null!=app.dmPostData.since_id;if(b&&0==a.length)alert("No new messages");else{$(".friendMessages").hide();console.log("handling dm response for "+a.length);if(1==a.length&&a[0].id_str==app.dmMaxId)app.hasFirstDirectMessage||($("#directMessages").append(Handlebars.getTemplate("no-more-direct-messages-template")()),
$("#olderDirectMessagesButtonContainer").hide()),app.hasFirstDirectMessage=!0,app.hideOverlays(),app.updatingDirectMessages=!1;else if(0==a.length)$("#directMessages").append(Handlebars.getTemplate("no-seecrets-in-direct-messages-template")()),app.hideOverlays(),app.updatingDirectMessages=!1;else{var c=app.getUniqueInstancesFromList(a,function(a){return a.sender.id_str});console.log("got "+c+" unique senders");var d=app.getSeecretMessageIndexesBySender(a,c);console.log("dechainifying the messages");
console.log("dechainifygin "+a.length);d=app.dechainifyDirectMessages(a,d);console.log("done dechainifying the messages");console.log("dechainifyinge  left "+d.length);app.dmMaxId=app.lastProcessedStatusId=d[d.length-1].id_str;app.timer("getting first incomplete start in dm list");c=app.getEarliestIncompleteDMChainStartId(d,c);app.timer("getting first incomplete start in dm list");0<c&&(app.dmMaxId=d[c].id_str);app.timer("Unhide and decompress timeline");d=app.unhideAndDecompressTimelineSeecrets(d,
!0);app.timer("Unhide and decompress timeline");d=app.imgUrlsToHttps(d,"sender");app.timelineContainsKeys(d)&&app.savePublicKeys(app.getPublicKeyMessagesFromDMList(d));c=app.getFriendsFromMessageList(d);app.markMessageableFriends(c);console.log("Found "+c.length+" friends");if(b){for(var e in c)null==$("#friendDetails_"+c[e].id_str)&&$("#friendsContainer").prepend([c[e]]);var b="New messages from ",g="";for(e in c)b+=g+c[e].screen_name,g=",";0<a.length&&alert(b)}else $("#friendsContainer").html(Handlebars.getTemplate("friends-list-template")(c)),
app.renderFriendMessages(c,d),$("#friendsContainer").fadeIn();app.timelineContainsEncryptedMessages(d)&&app.markEncryptedDirectMessages(d);app.updateDirectMessagesUI(d)}app.hideOverlays()}},updateDirectMessagesUI:function(a){app.hideOverlays();var b=app.getFriendsFromMessageList(a);if(app.dmPostData.since_id)for(var c in b){var d=app.filterMessageListByFriendId(a,b[c].id_str);console.log("found "+d.length+" messages from "+b[c].screen_name);0<d.length&&(console.log("rendering messages into #friendMessages_"+
b[c].id_str),$("#friendMessages_"+b[c].id_str).prepend(Handlebars.getTemplate("direct-messages-template")(d)))}else app.renderFriendMessages(b,a);app.updatingDirectMessages=!1},markMessageableFriends:function(a){var b=app.getObject("publicKeys-"+app.activeUser.user.id_str),c;for(c in a)console.log("the public key is "+b[a[c].id_str]),a[c].publicKey=b[a[c].id_str]},renderFriendMessages:function(a,b){for(var c in a)app.renderFriendMessage(a[c].id_str,b)},renderFriendMessage:function(a,b){var c=app.filterMessageListByFriendId(b,
a);$("#friendMessages_"+a).append(Handlebars.getTemplate("direct-messages-template")(c))},getFriendsFromMessageList:function(a){var b={},c;for(c in a)b[a[c].sender.screen_name]||(b[a[c].sender.screen_name]=a[c].sender,b[a[c].sender.screen_name].most_recent=a[c].seecret?a[c].seecret:a[c].text),b[a[c].sender.screen_name].message_count||(b[a[c].sender.screen_name].message_count=0),b[a[c].sender.screen_name].message_count++;a=[];for(f in b)a.push(b[f]);return a},toggleFriendMessages:function(a){"show conversation"!=
$("#friendMessagesToggleLink_"+a).html()?($("#friendMessagesToggleLink_"+a).html("show conversation"),$("#friendMessages_"+a).fadeOut()):($("#friendMessagesToggleLink_"+a).html("hide"),$("#friendMessages_"+a).fadeIn())},filterMessageListByFriendId:function(a,b){var c=[],d;for(d in a)a[d].sender.id_str!=b&&a[d].sender.id_str!=app.activeUser.id_str||c.push(a[d]);console.log("filtered "+a.length+" down to "+c.length);return c},publicKeyStartRegex:/^\s*-----BEGIN PGP PUBLIC KEY BLOCK-----\s/,publicKeyEndRegex:/\s-----END PGP PUBLIC KEY BLOCK-----\s*$/,
...
app.saveObject("publicKeys-"+app.activeUser.user.id_str,b);app.activeUser.publicKeys=b},hasFirstDirectMessage:!1,markEncryptedDirectMessages:function(a){for(var b in a)this.isPGPMessage(a[b].seecret)&&(a[b].encrypted=!0,a[b].decrypted=!1)},sendPublicKeyMessages:function(a,b){app.postDirectMessages(a,function(){app.trackSentPublicKeys(b)})},trackSentPublicKeys:function(a){var b=app.activeUser.sentPublicKeys;null==b&&(b={});for(var c in a)b[c]=new Date;app.activeUser.sentPublicKeys=b;app.saveObject("sentPublicKeys-"+
app.activeUser.user.id_str,b)},passphrase:null,decryptDirectMessages:function(a){var b=openpgp.key.readArmored(app.activeUser.privateKey.armored).keys[0];if(app.decryptPrivateKey(b))app.verifyMessagesDecrypted(a,b);else{alert("Could not verify your passphrase.  Showing messages undecrypted.");for(var c in a)a[c].encrypted&&(a[c].text+=" (could not decrypt)");app.updateDirectMessagesUI(a)}},verifyMessagesDecrypted:function(a,b){var c=null,d;for(d in a)if(a[d].encrypted&&0==a[d].decrypted){c=d;break}var e=
app.activeUser.user;if(null!=c)if(a[c].sender.id_str==e.id)a[c].decrypted=!0,app.verifyMessagesDecrypted(a,b);else{a[c].decrypted=!0;a[c].decryptionSuccessful=!1;a[c].originaltext=a[c].text;try{var g=openpgp.message.readArmored(a[c].seecret),h={privateKey:b,message:g};if(app.activeUser.publicKeys&&app.activeUser.publicKeys[a[c].sender.id_str]){var m=openpgp.key.readArmored(app.activeUser.publicKeys[a[c].sender.id_str].key);h.publicKeys=m.keys}try{openpgp.decrypt(h).then(function(b){a[d].decryptionSuccessful=
!0;a[d].seecret=b.data;b.signatures&&0<b.signatures.length&&b.signatures[0].valid&&(a[d].signed=!0);app.verifyMessagesDecrypted(a,h.privateKey)},function(b){a[d].text=app.generateUndecryptedMessageContent(a[d]);app.verifyMessagesDecrypted(a,h.privateKey)})}catch(l){console.log("try/catch error decrypting: "+JSON.stringify(l)),a[d].text=app.generateUndecryptedMessageContent(a[d]),app.verifyMessagesDecrypted(a,b)}}catch(l){console.log("major try/catch error decrypting: "+JSON.stringify(l)),a[d].text=
app.generateUndecryptedMessageContent(a[d]),app.verifyMessagesDecrypted(a,b)}}else{(c=app.activeUser.dmSenders)||(c={});for(var k in a)a[k].encrypted&&a[k].sender.id_str!=app.activeUser.id&&(c[a[k].sender.id_str]=!0);app.saveObject("dmSenders-"+app.activeUser.user.id_str,c);app.refreshActiveUserDMSenders();app.updateDirectMessagesUI(a)}},undecryptedMessageContentHandler:null,generateUndecryptedMessageContent:function(a){return Handlebars.getTemplate("undecrypted-message-content-template")(a)},showPublicKeyManager:function(a,
...
console.log(JSON.stringify(a))}))},generatePrivateKeyPairSuccess:function(a,b,c,d){c||delete b.passphrase;c=app.activeUser.user;var e=new Date,g={};g.armored=a.privateKeyArmored;g.dateCreated=e;var h={};h.armored=a.publicKeyArmored;h.dateCreated=e;app.saveObject("keyOptions-"+c.id_str,b);app.saveObject("privateKey-"+c.id_str,g);app.saveObject("publicKey-"+c.id_str,h);app.checkDirectMessageStatus();app.refreshActiveUserTotal();d()},showAllDirectMessages:function(){$("#friendName").html("");$(".dmMessageDiv").fadeIn();
$("#directMessagesActionsContainer").hide();$("body").scrollTop(0)},setDirectMessageReceiver:function(a,b){$("#selectedReceiverId").val(a);$("#selectedReceiverName").val(b);var c={};c.name=b;c.id_str=a;$("#friendName").html(Handlebars.getTemplate("direct-message-friend-name")(c));app.checkDirectMessageStatus();$(".dmMessageDiv").hide();$(".dm_"+a).fadeIn();$("#directMessagesActionsContainer").show();$("body").scrollTop(0)},initiateDirectMessages:function(a,b){var c=app.generatePublicKeyMessages(a),
d=[],e;for(e in c)d.push({receiverId:a,text:c[e]});b||d.push({text:app.joinMessage,receiverId:a});app.postDirectMessages(d,function(){var b={};b[a]=!0;app.trackSentPublicKeys(b);app.checkDirectMessageStatus()})},joinMessage:"I'm using Seecret to send encrypted direct messages over Twitter. Try it with me! Just go to https://www.seecret.net",sendingDirectMessage:!1,prepareDirectMessage:function(a){if(!app.sendingDirectMessage){var b=$("#directMessage").val();0!=b.length&&(app.sendingDirectMessage=
!0,app.overlay(),app.encryptDirectMessage(b,a))}},decryptPrivateKey:function(a){var b=app.activeUser.keyOptions,c=null;app.passphrase&&(c=app.passphrase);b.passphrase&&(c=b.passphrase);b.passphrase||null!=app.passphrase||(app.passphrase=c=prompt("Enter your passphrase"));if(a.decrypt(c))return!0;app.passphrase=null;return!1},encryptDirectMessage:function(a,b){var c=app.activeUser.publicKeys,d=null,e;for(e in c)e==b&&(d=c[e].key);d?(c=openpgp.key.readArmored(d).keys,c={publicKeys:c,data:a},app.activeUser.privateKey&&
(d=openpgp.key.readArmored(app.activeUser.privateKey.armored),app.decryptPrivateKey(d.keys[0])&&(c.privateKeys=d.keys)),openpgp.encrypt(c).then(function(a){a=app.hideAndCompress(a.data);app.seecret_engine.config.RANDOM_COVERTEXTS=!0;app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH=1E4;a=app.seecret_engine.chainify(a,covertexts);app.seecret_engine.config.RANDOM_COVERTEXTS=!1;app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH=140;a.reverse();for(var c in a)a[c]={receiverId:b,text:a[c]};app.postDirectMessages(a,
function(){});$("#directMessage").val("")}).catch(function(a){console.log("Error generating the messages:"+JSON.stringify(a));app.hideOverlays();app.sendingDirectMessage=!1})):(app.sendingDirectMessage=!1,app.hideOverlays(),alert("You do not have that users public key."))},postedDirectMessageId:null,directMessagesToPost:null,directMessageCount:0,postDirectMessages:function(a,b){app.directMessagesToPost=a;app.directMessageCount=0;app.checkDirectMessages(b)},checkDirectMessages:function(a){if(app.directMessagesToPost&&
...
app.handleTimelineResponse)},getTimeline:function(a,b){app.oauthResult.get("1.1/statuses/home_timeline.json",a).done(function(a){b(a);app.hideOverlays()}).fail(app.handleGetTimelineError)},getUserTimeline:function(a,b){$("#twitterUser").val("");app.oauthResult.get("1.1/statuses/user_timeline.json",a).done(function(a){b(a);app.hideOverlays()}).fail(app.handleGetTimelineError)},handleTimelineResponse:function(a){0<a.length?1==a.length&&a[0].id_str==app.maxId?$("#status-list").append(Handlebars.getTemplate("no-more-timeline-message-template")()):
...
app.processTimelineWithFollowerInfo)},addFollowerTagToTweets:function(a,b){var c={},d;for(d in a)c[a[d].user.id_str]=a[d].user.id_str;d=[];for(var e in c)d.push(e);c={data:{}};c.data.user_id=d.join();app.oauthResult.get("1.1/friendships/lookup.json",c).done(function(c){var d={},e;for(e in c)0<=c[e].connections.indexOf("followed_by")&&(d[c[e].id_str]=!0);for(var k in a)d[a[k].user.id_str]&&(a[k].follower=!0);b(a)}).fail(function(c){b(a)})},unhideAndDecompressTimelineSeecrets:function(a,b){var c=[],
d;for(d in a)if(a[d].seecret_envelope){var e=a[d].seecret_envelope,g=app.seecret_engine.getSeecretFromEnvelope(e),e=app.seecret_engine.getContentTypeFromEnvelope(e),g=app.seecret_engine.unhide(g,e);if(e==app.seecret_engine.config.CONTENT_TYPES.NUMBERS_ARRAY)try{g=seecret_compression.decompress(g),a[d].seecret=g}catch(h){console.log("error decompressing a seecret message: "+JSON.stringify(h)),a[d].seecret="Could not decompress the Seecret",a[d].seecret_error=h}else a[d].seecret=g;c.push(a[d])}else b&&
...
$(".basicTweet").fadeIn();app.updatingTimeline=!1;app.userTimelineId?app.renderUserTimelineHeader(app.userTimelineId):$("#timelineName").html("")},renderUserTimelineHeader:function(a){$("#timelineName").html("for "+a+" &nbsp;&nbsp;&nbsp;<a href='javascript:app.initTimeline();app.updateHomeTimeline()'>Back to my timeline</a>")},dechainifyTweets:function(a,b){for(var c in b)if(0<b[c].length){var d=0,e;for(e in b[c]){var g=app.seecret_engine.dechainify(a,{chainSegmentMatcher:function(a){return a&&a.text&&
"string"==typeof a.text&&a.user&&a.user.id_str==c?a:null},chainSegmentContentFinder:this.chainSegmentContentFinder,startIndex:b[c][e],withCovertexts:!0});g&&""!=g.seecret&&(a[b[c][e]].seecret_envelope=g.seecret,a[b[c][e]].covertexts=g.covertexts);d++}}return a},dechainifyDirectMessages:function(a,b){for(var c in b)if(0<b[c].length){var d=function(a){return a&&a.text&&"string"==typeof a.text&&a.sender&&a.sender.id_str==c?a:null},e=0,g;for(g in b[c]){var h=app.seecret_engine.dechainify(a,{chainSegmentMatcher:d,
chainSegmentContentFinder:this.chainSegmentContentFinder,startIndex:b[c][g],withCovertexts:!0});h&&""!=h.seecret&&(a[b[c][g]].seecret_envelope=h.seecret,a[b[c][g]].covertexts=h.covertexts);e++}}return a},chainSegmentContentFinder:function(a){return a.text},tweetSenderGetter:function(a){return a.user?a.user.id_str:null},dmSenderGetter:function(a){return a.user?a.user.id_str:null},getMessageIndexes:function(a,b,c){var d={},e;for(e in b)d[e]=app.seecret_engine.getOrdinalIndexes(a,{chainSegmentMatcher:function(a){return c(a)==
...
timers:{},timer:function(a){var b=moment();app.timers[a]?(console.log(a+" took "+b.diff(app.timers[a])+" milliseconds"),delete app.timers[a]):app.timers[a]=b}};
