...
app.updatingDirectMessages=!1):(a=app.processMessageList(a,"sender","dmMaxId"),app.timelineContainsKeys(a)&&app.savePublicKeys(app.getPublicKeyMessagesFromDMList(a)),app.timelineContainsEncryptedMessages(a)?(app.markEncryptedDirectMessages(a),app.decryptDirectMessages(a)):(app.createConversationEntries(a),app.updateDirectMessagesUI(a,friendList)));app.hideOverlays();app.timer("handleDMResponse")},createConversationEntries:function(a){var b=app.getFriendsFromMessageList(a);if(null!=app.dmPostData.since_id){for(var c=
"New messages from ",d="",e=0;e<b.length;e++)c+=d+b[e].screen_name,d=",";0<a.length&&alert(c);for(e=0;e<b.length;e++)b[e].newMessages=!0,b[e].newMessageCount=8<b[e].message_count?9:b[e].message_count}app.markMessageableFriends(b);app.markFriendsWithSentKeys(b);app.markFriendsWithUnencryptedMessages(b,a);if($("#friendsContainer").is(":empty"))$("#friendsContainer").html(Handlebars.getTemplate("conversations-template")(b));else{a=[];for(e=0;e<b.length;e++)if(null==document.getElementById("friendDetails_"+
...
updateDirectMessagesUI:function(a,b){app.hideOverlays();b=b?b:app.getFriendsFromMessageList(a);app.renderAllFriendMessages(b,a);app.updatingDirectMessages=!1},markMessageableFriends:function(a){for(var b=app.getObject("publicKeys-"+app.activeUser.user.id_str),c=0;c<a.length;c++)a[c].publicKey=b[a[c].id_str]},markFriendsWithSentKeys:function(a){var b=app.getObject("sentPublicKeys-"+app.activeUser.user.id_str),c;for(c in b)for(var d=0;d<a.length;d++)c==a[d].id_str&&(a[d].sentKey=b[c])},markFriendsWithUnencryptedMessages:function(a,
b){for(var c=0;c<a.length;c++){a[c].unencryptedMessages=!1;for(var d=0;d<b.length;d++)if(b[d].encrypted&&!b[d].encryptionSuccessful){a[c].unencryptedMessages=!0;break}}},renderAllFriendMessages:function(a,b){for(var c=0;c<a.length;c++)app.renderFriendMessages(a[c].id_str,b)},renderFriendMessages:function(a,b){var c=app.filterMessageListByFriendId(b,a);if(app.dmPostData.since_id){for(var d=0;d<c.length;d++)c[d].new=!0;$("#friendMessages_"+a).prepend(Handlebars.getTemplate("direct-messages-template")(c))}else $("#friendMessages_"+
a).append(Handlebars.getTemplate("direct-messages-template")(c))},getFriendsFromMessageList:function(a){for(var b={},c=0;c<a.length;c++)b[a[c].sender.screen_name]||(b[a[c].sender.screen_name]=a[c].sender,b[a[c].sender.screen_name].most_recent=a[c]),b[a[c].sender.screen_name].message_count||(b[a[c].sender.screen_name].message_count=0),b[a[c].sender.screen_name].message_count++;a=[];for(var d in b)a.push(b[d]);return a},toggleFriendMessages:function(a){"show conversation"!=$("#friendMessagesToggleLink_"+
a).html()?($("#friendMessagesToggleLink_"+a).html("show conversation"),$("#friendMessageForm_"+a).fadeOut(),$("#friendMessages_"+a).fadeOut()):($("#friendMessagesToggleLink_"+a).html("hide conversation"),$("#friendMessageForm_"+a).fadeIn(),$("#friendMessages_"+a).fadeIn())},filterMessageListByFriendId:function(a,b){for(var c=[],d=0;d<a.length;d++)a[d].sender.id_str!=b&&a[d].sender.id_str!=app.activeUser.id_str||c.push(a[d]);return c},getOlderDirectMessages:function(){app.hasFirstDirectMessage||(app.setupDMPostData(),
app.dmPostData.max_id=app.dmMaxId,app.doDMPost(app.dmPostData,function(a){if(a.length<app.NUM_DM||app.dmMaxId==a[a.length-1].id_str)app.hasFirstDirectMessage=!0,$("#directMessages").append(Handlebars.getTemplate("no-more-direct-messages-template")()),$("#getOlderMessagesButton").hide();app.handleDMResponse(a)}))},isPublicKey:function(a){return app.publicKeyStartRegex.test(a)&&app.publicKeyEndRegex.test(a)},isPGPMessage:function(a){return app.pgpMessageStartRegex.test(a)&&app.pgpMessageEndRegex.test(a)},
timelineContainsKeys:function(a){for(var b in a)if(app.isPublicKey(a[b].seecret))return!0;return!1},timelineContainsEncryptedMessages:function(a){for(var b in a)if(app.isPGPMessage(a[b].seecret))return!0;return!1},getPublicKeyMessagesFromDMList:function(a){for(var b=[],c=0;c<a.length;c++)app.isPublicKey(a[c].seecret)&&b.push(a[c]);return b},savePublicKeys:function(a){var b=app.activeUser.publicKeys;null==b&&(b={});for(var c={},d=0;d<a.length;d++)a[d].sender.id_str!=app.activeUser.user.id&&(c[a[d].sender.id_str]||
(c[a[d].sender.id_str]={key:a[d].seecret,screen_name:a[d].sender.screen_name,id_str:a[d].sender.id_str,profile_image_url:a[d].sender.profile_image_url,receiveDate:a[d].created_at}));for(var e in c)b[e]||$("#friendKeyContainer_"+e).html("Just sent you their key!"),b[e]=c[e];app.saveObject("publicKeys-"+app.activeUser.user.id_str,b);app.activeUser.publicKeys=b},markEncryptedDirectMessages:function(a){for(var b=0;b<a.length;b++)this.isPGPMessage(a[b].seecret)&&(a[b].encrypted=!0,a[b].decrypted=!1)},
sendPublicKeyMessages:function(a,b){app.postDirectMessages(a,function(){app.trackSentPublicKeys(b)})},trackSentPublicKeys:function(a){var b=app.activeUser.sentPublicKeys;null==b&&(b={});for(var c in a)b[c]=new Date;app.activeUser.sentPublicKeys=b;app.saveObject("sentPublicKeys-"+app.activeUser.user.id_str,b)},decryptDirectMessages:function(a){var b=openpgp.key.readArmored(app.activeUser.privateKey.armored).keys[0];if(app.decryptPrivateKey(b))app.timer("decryptDirectMessages"),app.verifyMessagesDecrypted(a,
b);else{alert("Could not verify your passphrase.  Showing messages undecrypted.");for(b=0;b<a.length;b++)a[b].encrypted&&(a[b].text+=" (could not decrypt)");app.updateDirectMessagesUI(a)}},verifyMessagesDecrypted:function(a,b){for(var c=null,d=0;d<a.length;d++)if(a[d].encrypted&&0==a[d].decrypted){c=d;break}var e=app.activeUser.user;if(null!=c)if(a[c].sender.id_str==e.id)a[c].decrypted=!0,app.verifyMessagesDecrypted(a,b);else{a[c].decrypted=!0;a[c].decryptionSuccessful=!1;a[c].originaltext=a[c].text;
try{var f=openpgp.message.readArmored(a[c].seecret),h={privateKey:b,message:f};if(app.activeUser.publicKeys&&app.activeUser.publicKeys[a[c].sender.id_str]){var g=openpgp.key.readArmored(app.activeUser.publicKeys[a[c].sender.id_str].key);h.publicKeys=g.keys}try{openpgp.decrypt(h).then(function(b){a[d].decryptionSuccessful=!0;a[d].seecret=b.data;b.signatures&&0<b.signatures.length&&b.signatures[0].valid&&(a[d].signed=!0);app.verifyMessagesDecrypted(a,h.privateKey)},function(b){a[d].text=app.generateUndecryptedMessageContent(a[d]);
app.verifyMessagesDecrypted(a,h.privateKey)})}catch(k){console.log("try/catch error decrypting: "+JSON.stringify(k)),a[d].text=app.generateUndecryptedMessageContent(a[d]),app.verifyMessagesDecrypted(a,b)}}catch(k){console.log("major try/catch error decrypting: "+JSON.stringify(k)),a[d].text=app.generateUndecryptedMessageContent(a[d]),app.verifyMessagesDecrypted(a,b)}}else{app.timer("decryptDirectMessages");(c=app.activeUser.dmSenders)||(c={});for(e=0;e<a.length;e++)a[e].encrypted&&a[e].sender.id_str!=
app.activeUser.id&&(c[a[e].sender.id_str]=!0);app.saveObject("dmSenders-"+app.activeUser.user.id_str,c);app.refreshActiveUserDMSenders();app.timer("updateDirectMessagesUI");app.createConversationEntries(a);app.updateDirectMessagesUI(a);app.timer("updateDirectMessagesUI")}},undecryptedMessageContentHandler:null,generateUndecryptedMessageContent:function(a){return Handlebars.getTemplate("undecrypted-message-content-template")(a)},showPublicKeyManager:function(a,b){app.doScroll=!1;var c={};c.receiver_name=
...
app.doScroll=!1},makeKeyOptionsForUser:function(a){var b=app.generatePassword(),c=1024;openpgp.util.getWebCryptoAll()&&(c=2048);return{numBits:c,userIds:[{name:a.name,email:a.screen_name+"@seecretApp.seecret"}],passphrase:b}},generatePassword:function(){for(var a="",b=0;50>b;b++)a+=app.getValidPasswordCharacter();return a},getValidPasswordCharacter:function(){for(;;){var a=Math.floor(206*Math.random())+48;if(!(173==a||92==a||58<=a&&64>=a||127<=a&&160>=a))return String.fromCharCode(a)}},validateNewPrivateKey:function(a){if(1>
$("#"+a).val().length)return $("#"+a+"Error").html("You must have a passphrase for your key of at least one character"),!1;$("#"+a+"Error").html("");if(confirm("Are you sure?")){var b=$("#"+a).val(),c=1==document.getElementById(a+"Cache").checked;app.hideAllViews();$("#"+app.STATUS_MESSAGE_VIEW).show();app.generatePrivateKeyPair(b,c,function(){$("#"+a).val("");confirm("Send your new key to the contacts who have the old one?")&&app.resendPublicKeys();$("#directMessagesActionsContainer").empty();app.renderPrivateKeySettings();
app.setView(app.SETTINGS_VIEW)})}},resendPublicKeys:function(){var a=app.activeUser.sentPublicKeys,b=app.activeUser.dmSenders;if(null!=a||null!=b){var c=[],d={},e;for(e in a)d[e]=!0;for(e in b)d[e]=!0;var a=0,f;for(f in d)for(a++,b=app.generatePublicKeyMessages(f),e=0;e<b.length;e++)c.push({receiverId:f,text:b[e]});0<c.length&&app.sendPublicKeyMessages(c,d)}},generatePrivateKeyPair:function(a,b,c){var d=app.activeUser.user,e=app.makeKeyOptionsForUser(d);e.passphrase=a;d.id_str||(e.userId=d.id_str);
null!=d&&(a=new Date,console.log("start generating key pair : "+a.getSeconds()+":"+a.getMilliseconds()),openpgp.config.debug=!0,openpgp.generateKey(e).then(function(a){var d=new Date;console.log("done generating key pair : "+d.getSeconds()+":"+d.getMilliseconds());app.generatePrivateKeyPairSuccess(a,e,b,c)}).catch(function(a){console.log("Caught an error generatinga key pair");console.log(JSON.stringify(a))}))},generatePrivateKeyPairSuccess:function(a,b,c,d){c||delete b.passphrase;c=app.activeUser.user;
var e=new Date,f={};f.armored=a.privateKeyArmored;f.dateCreated=e;var h={};h.armored=a.publicKeyArmored;h.dateCreated=e;app.saveObject("keyOptions-"+c.id_str,b);app.saveObject("privateKey-"+c.id_str,f);app.saveObject("publicKey-"+c.id_str,h);app.checkDirectMessageStatus();app.refreshActiveUserTotal();d()},showAllDirectMessages:function(){$(".friendMessages").show()},invite:function(a,b){for(var c=app.generatePublicKeyMessages(a),d=[],e=0;e<c.length;e++)d.push({receiverId:a,text:c[e]});b||d.push({text:app.joinMessage,
receiverId:a});app.overlay();app.postDirectMessages(d,function(){var b={};b[a]=!0;app.trackSentPublicKeys(b);$("#friendKeyContainer_"+a).html("Invite sent.")})},prepareDirectMessage:function(a){if(!app.sendingDirectMessage){var b=$("#directMessage_"+a).val();0!=b.length&&($("#directMessage_"+a).val(""),app.sendingDirectMessage=!0,app.overlay(),app.encryptDirectMessage(b,a))}},decryptPrivateKey:function(a){var b=app.activeUser.keyOptions,c=null;app.passphrase&&(c=app.passphrase);b.passphrase&&(c=b.passphrase);
b.passphrase||null!=app.passphrase||(app.passphrase=c=prompt("Enter your passphrase"));if(a.decrypt(c))return!0;app.passphrase=null;return!1},encryptDirectMessage:function(a,b){var c=app.activeUser.publicKeys,d=null,e;for(e in c)e==b&&(d=c[e].key);d?(c=openpgp.key.readArmored(d).keys,c={publicKeys:c,data:a},app.activeUser.privateKey&&(d=openpgp.key.readArmored(app.activeUser.privateKey.armored),app.decryptPrivateKey(d.keys[0])&&(c.privateKeys=d.keys)),openpgp.encrypt(c).then(function(a){a=app.hideAndCompress(a.data);
app.seecret_engine.config.RANDOM_COVERTEXTS=!0;app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH=1E4;a=app.seecret_engine.chainify(a,covertexts);app.seecret_engine.config.RANDOM_COVERTEXTS=!1;app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH=140;a.reverse();for(var c=0;c<a.length;c++)a[c]={receiverId:b,text:a[c]};app.postDirectMessages(a,function(){});$("#directMessage").val("")}).catch(function(a){console.log("Error generating the messages:"+JSON.stringify(a));app.hideOverlays();app.sendingDirectMessage=
!1})):(app.sendingDirectMessage=!1,app.hideOverlays(),alert("You do not have that users public key."))},postDirectMessages:function(a,b){app.overlay();app.directMessagesToPost=a;app.directMessageCount=0;app.checkDirectMessages(b)},checkDirectMessages:function(a){if(app.directMessagesToPost&&app.directMessageCount<app.directMessagesToPost.length){var b=app.directMessagesToPost[app.directMessageCount].text,c=app.directMessagesToPost[app.directMessageCount].receiverId;app.directMessageCount++;app.postDirectMessage(b,
c,a)}else app.hideOverlays(),alert("Sent!"),app.sendingDirectMessage=!1,a&&a()},postDirectMessage:function(a,b,c){app.oauthResult.post("1.1/direct_messages/new.json",{data:{text:a,user_id:b}}).done(function(a){app.checkDirectMessages(c)}).fail(function(a){})},generatePublicKeyMessages:function(a){a=app.activeUser.publicKey;if(null!=a)return a=app.hideAndCompress(a.armored),app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH=1E4,chain=app.seecret_engine.chainify(a,["Here's my Seecret key","Now we can start using Seecret to send encrypted messages on Twitter."]),
...
{},d=0;d<a.length;d++)c[a[d].user.id_str]=a[d].user.id_str;var d=[],e;for(e in c)d.push(e);c={data:{}};c.data.user_id=d.join();app.oauthResult.get("1.1/friendships/lookup.json",c).done(function(c){for(var d={},e=0;e<c.length;e++)0<=c[e].connections.indexOf("followed_by")&&(d[c[e].id_str]=!0);for(c=0;c<a.length;c++)d[a[c].user.id_str]&&(a[c].follower=!0);b(a)}).fail(function(c){b(a)})},unhideAndDecompressSeecretsInList:function(a,b){for(var c=[],d=0;d<a.length;d++)if(a[d].seecret_envelope||!document.getElementById("showOnlySeecrets").checked)if(a[d].seecret_envelope){var e=
a[d].seecret_envelope,f=app.seecret_engine.getSeecretFromEnvelope(e),e=app.seecret_engine.getContentTypeFromEnvelope(e),f=app.seecret_engine.unhide(f,e);if(e==app.seecret_engine.config.CONTENT_TYPES.NUMBERS_ARRAY)try{f=seecret_compression.decompress(f),a[d].seecret=f}catch(h){a[d].seecret="Could not decompress the Seecret",a[d].seecret_error=h}else a[d].seecret=f;c.push(a[d])}else b&&!app.seecret_engine.hasSeecretContent(a[d].text)&&c.push(a[d]);return c},processMessageList:function(a,b,c){var d=
function(a){return a[b]?a[b].id_str:null},e=app.getUniqueInstancesFromList(a,d),f=app.getMessageIndexes(a,e,d);a=app.dechainifyMessages(a,f,b);app[c]=app.lastProcessedStatusId=a[a.length-1].id_str;var f=0,h;for(h in e){for(var g=e=0;g<a.length;g++)d(a[g])==h&&app.seecret_engine.hasSeecretContent(a[g].text)&&(app.seecret_engine.isEnvelopeStart(a[g].text)&&(e=g),app.seecret_engine.isEnvelopeEnd(a[g].text)&&(e=0));0<e&&0==f?f=e:0<e&&e<f&&(f=e)}d=f;0<d&&(app[c]=a[d].id_str);a=app.unhideAndDecompressSeecretsInList(a,
!0);for(g=0;g<a.length;g++)a[g][b]&&a[g][b].profile_image_url&&(a[g][b].profile_image_url=a[g][b].profile_image_url.replace(app.httpsReplaceRegex,"https:"));return a},processTimelineWithFollowerInfo:function(a){var b=null;app.dmPost&&app.dmPost.since_id&&app.dmMaxId&&(b=app.dmMaxId);a=app.processMessageList(a,"user","maxId");b&&(app.dmMaxId=b);0<a.length?$("#status-list").append(Handlebars.getTemplate("status-template")(a)):document.getElementById("showOnlySeecrets").checked&&$("#status-list").append(Handlebars.getTemplate("no-seecrets-in-timeline-segment-template")());
$(".basicTweet").fadeIn();app.updatingTimeline=!1;app.userTimelineId?app.renderUserTimelineHeader(app.userTimelineId):$("#timelineName").html("")},renderUserTimelineHeader:function(a){$("#timelineName").html("for "+a+" &nbsp;&nbsp;&nbsp;<a href='javascript:app.initTimeline();app.updateHomeTimeline()'>Back to my timeline</a>")},dechainifyMessages:function(a,b,c){for(var d in b)if(0<b[d].length){var e=function(a){return a&&a.text&&"string"==typeof a.text&&a[c]&&a[c].id_str==d?a:null},f=0,h;for(h in b[d]){var g=
app.seecret_engine.dechainify(a,{chainSegmentMatcher:e,chainSegmentContentFinder:this.chainSegmentContentFinder,startIndex:b[d][h],withCovertexts:!0});g&&""!=g.seecret&&(a[b[d][h]].seecret_envelope=g.seecret,a[b[d][h]].covertexts=g.covertexts);f++}}return a},chainSegmentContentFinder:function(a){return a.text},tweetSenderGetter:function(a){return a.user?a.user.id_str:null},dmSenderGetter:function(a){return a.user?a.user.id_str:null},getMessageIndexes:function(a,b,c){var d={},e;for(e in b)d[e]=app.seecret_engine.getOrdinalIndexes(a,
{chainSegmentMatcher:function(a){return c(a)==b[e]?a:null},chainSegmentContentFinder:this.chainSegmentContentFinder});return d},getSeecretMessageIndexesByUser:function(a,b){var c={},d;for(d in b)c[d]=app.seecret_engine.getOrdinalIndexes(a,{chainSegmentMatcher:function(a){return a&&a.text&&"string"==typeof a.text&&a.user&&a.user.id_str==b[d]?a:null},chainSegmentContentFinder:this.chainSegmentContentFinder});return c},getSeecretMessageIndexesBySender:function(a,b){var c={},d;for(d in b)c[d]=app.seecret_engine.getOrdinalIndexes(a,
{chainSegmentMatcher:function(a){return a&&a.text&&"string"==typeof a.text&&a.sender&&a.sender.id_str==b[d]?a:null},chainSegmentContentFinder:this.chainSegmentContentFinder});return c},getUniqueTweeters:function(a){return app.getUniqueInstancesFromList(a,app.getTweeterId)},getUniqueDMSenders:function(a){return app.getUniqueInstancesFromList(a,app.getDMSenderId)},getTweeterId:function(a){return a.user.id_str},getDMSenderId:function(a){return a.sender.id_str},getUniqueInstancesFromList:function(a,b){for(var c=
{},d=0;d<a.length;d++){var e=b(a[d]);e&&(c[e]=e)}return c},generateTimelinePostVals:function(){var a={data:{}};a.data.count=200;null!=app.maxId&&(a.data.max_id=app.maxId);return a},initTimeline:function(){app.maxId=null;app.homeTimeline=!0;app.userTimelineId=null;app.updateHomeTimeline()},updateHomeTimeline:function(a){$("#twitterUser").val("");a&&(app.maxId=null);app.homeTimeline=!0;$("#timelineName").empty();app.updatingTimeline=!0;a=app.generateTimelinePostVals();a.data.max_id||($("#status-list").empty(),
app.lastProcessedStatusId=null);app.overlay();app.timer("Get timeline");app.getTimeline(a,app.handleTimelineResponse)},prepareUpdateMessage:function(){var a=$("#textarea-statusUpdate").val(),a=app.hideAndCompress(a),b=null;app.useAutogeneratedScaffolding()||(b=app.getScaffolds());a=app.seecret_engine.chainify(a,b);a.reverse();return a},updateStatus:function(){app.overlay();var a=this.prepareUpdateMessage();this.postChain(a)},postChain:function(a){this.currentMessageChain=a;this.chainCount=0;this.postChainItem()},
postChainItem:function(){this.chainCount<this.currentMessageChain.length?app.oauthResult.post("1.1/statuses/update.json",{data:{status:this.currentMessageChain[this.chainCount++]}}).done(function(a){setTimeout(function(){app.postChainItem()},50)}).fail(function(a){}):($("#status-list").empty(),$("#scaffoldingWarningContainer").empty(),$("#scaffoldingStatus").empty(),$("#textarea-statusUpdate").val(""),$("#textarea-statusUpdate").val(""),$("#scaffoldsContainer").html(Handlebars.getTemplate("scaffolds-template")()),
...
$("#importPrivateKeyText").val("");$("#importPrivateKeyPassphrase").val("");$("#privateKeyTextarea").val("");$("#privateKeyPassphrase").val("");$("#privateKeyCopyarea").empty();$("#publicKeysDisplayArea").empty();app.unauthenticatedHeader();$("#privateMessagesButton").hide();$("#timelineButton").hide();app.unauthenticatedMenu();app.setView(app.ABOUT_VIEW);app.hideOverlays()},authenticatedMenu:function(){$("#privateMessagesButton").show();$("#timelineButton").show();$("#settingsButton").show();$("#logoutButton").show();
$("#hashesButton").show();$("#publicKeysButton").show();$("#aboutButton").show()},unauthenticatedMenu:function(){$("#privateMessagesButton").hide();$("#timelineButton").hide();$("#settingsButton").hide();$("#logoutButton").hide();$("#publicKeysButton").hide()},authenticatedHeader:function(){app.activeUser&&app.activeUser.user&&(app.activeUser.user.profile_image_url=app.activeUser.user.profile_image_url.replace(app.httpsReplaceRegex,"https:"),$("#loginContainer").html(Handlebars.getTemplate("authenticated-header-template")(app.activeUser.user)))},
...
return}e=app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH-(c[0].trim().length+a.length);e*=-1}else{var f=scaffoldLength=0;for(s=0;s<c.length;s++)2<=c[s].trim().length&&(scaffoldLength+=c[s].trim().length,f++);scaffoldLength+a.length<app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH*f?(b=!0,d=app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH*f-(scaffoldLength+a.length)):(e=app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH*f-(scaffoldLength+a.length),e*=-1)}b?app.makeUpdateFormReady(d):app.makeUpdateFormNotReady(e)}},
...
var b={};b.spillover=a;$("#scaffoldingStatus").html(Handlebars.getTemplate("scaffold-status-spillover-template")(b))},makeUpdateFormPending:function(){$("#submitButtonContainer").hide();$("#scaffoldingStatus").html(Handlebars.getTemplate("scaffold-status-incomplete-template")())},removeScaffold:function(a){for(var b=app.getScaffolds(),c=[],d=0;d<b.length;d++)each!=a&&c.push(b[each]);app.renderScaffolds(c);app.processScaffolding()},renderScaffolds:function(a){$("#scaffoldsContainer").html("");for(var b=
0,c=0;c<a.length;c++)app.addScaffoldField(b++,a[c]);$("#scaffoldRemover0").remove()},getScaffolds:function(){for(var a=[],b=document.getElementById("scaffold0");b;)a.push(b.value.trim()),b=document.getElementById("scaffold"+a.length);return a},toggleScaffolds:function(){document.getElementById("scaffoldingOption").checked?($("#scaffoldingStatus").html(""),$("#scaffoldsContainer").html(""),$("#scaffoldingWarningContainer").hide(),$("#scaffoldingContainer").hide(),$("#submitButtonContainer").show()):
($("#scaffoldingStatus").html(""),$("#scaffoldsContainer").html(""),$("#scaffoldingContainer").show(),$("#scaffoldingWarningContainer").html(""),$("#submitButtonContainer").hide(),app.addScaffoldField(0),app.processScaffolding())},getScaffoldTotal:function(a){for(var b=0,c=0;c<a.length;c++)b+=a[c].trim().length;return b},showCopyPrivateKey:function(a){"block"==$("#privateKeyCopyarea").css("display")?($(a).html("Show"),$("#privateKeyCopyarea").hide(),$("#privateKeyCopyarea").html("")):($(a).html("Hide"),
app.renderPrivateKeySettings())},toggleDeleteSettings:function(a){"block"==$("#deleteKeyForm").css("display")?($(a).html("Show"),$("#deleteKeyForm").hide()):($(a).html("Hide"),$("#deleteKeyForm").show())},renderPrivateKeySettings:function(){var a=app.activeUser.privateKey,b=app.activeUser.publicKey,c=app.activeUser.keyOptions,d={};app.activeUser&&app.activeUser.privateKey&&(d.privateKey=a,d.privateKey.dateCreatedDisplay=(new moment(d.privateKey.dateCreated)).format("MMMM Do YYYY, h:mm:ss a"),d.publicKey=
b,d.keyOptions=c);a=Handlebars.getTemplate("user-keys-template")(d);$("#privateKeyCopyarea").html(a);$("#privateKeyCopyarea").fadeIn()},showPublicKeys:function(a){"block"==$("#publicKeysDisplayArea").css("display")?($(a).html("Show"),$("#publicKeysDisplayArea").hide()):($(a).html("Hide"),app.renderPublicKeys(),$("#publicKeysDisplayArea").fadeIn())},showImportPrivateKey:function(a){"block"==$("#importForm").css("display")?($(a).html("Show"),$("#importForm").hide()):($(a).html("Hide"),$("#importForm").show())},
showGeneratePrivateKey:function(a){"block"==$("#generateForm").css("display")?($(a).html("Show"),$("#generateForm").hide()):($(a).html("Hide"),$("#generateForm").show())},saveObject:function(a,b){localStorage[a]=JSON.stringify(b)},getObject:function(a){return app.getLocalObject(a)},getLocalObject:function(a){return localStorage[a]?JSON.parse(localStorage[a]):null},saveLocalObject:function(a,b){localStorage[a]=JSON.stringify(b)},deleteSavedItem:function(a){delete localStorage[a]},importPrivateKey:function(){var a=
app.activeUser.user,b=$("#importPrivateKeyText").val(),c=new Date,d=openpgp.key.readArmored(b);if(!d||!d.keys||1>d.keys.length)alert("The key is invalid");else{var e=openpgp.key.readArmored(b).keys[0],f=$("#importPrivateKeyPassphrase").val();e.decrypt(f)?(f=$("#importPrivateKeyPassphrase").val(),$("#importPrivateKeyText").val(""),$("#importPrivateKeyPassphrase").val(""),d=app.makeKeyOptionsForUser(a),d.passphrase=f,b={armored:b,dateCreated:c},c={armored:e.toPublic().armor(),dateCreated:c},app.activeUser.privateKey=
...
deleteSavedPassphrase:function(){if(confirm("Are you sure?")){var a=app.activeUser.user,b=app.activeUser.keyOptions;b.passphrase="";app.activeUser.keyOptions=b;app.saveObject("keyOptions-"+a.id_str,b)}app.renderPrivateKeySettings()},changePassphrase:function(){var a=prompt("Enter new passphrase");if(null!=a){var b=prompt("Enter it again");null!=b&&(a==b?(alert("Changing your passphrase to "+b+"\n we urge you to delete it from this device once you've got it memorized or saved elsewhere."),app.setNewPassphraseOnPrivateKey(a)):
alert("Your new passphrase did not match, please try again"))}app.renderPrivateKeySettings()},setNewPassphraseOnPrivateKey:function(a){var b=app.activeUser.user,c=app.activeUser.keyOptions,d=openpgp.key.readArmored(app.activeUser.privateKey.armored);if(app.decryptPrivateKey(d.keys[0])){d.keys[0].primaryKey.encrypt(a);d.keys[0].subKeys&&0<d.keys[0].subKeys.length&&d.keys[0].subKeys[0]&&d.keys[0].subKeys[0].subKey.encrypt(a);var e={};e.dateCreated=new Date;e.armored=d.keys[0].armor();var f={};f.dateCreated=
new Date;f.armored=d.keys[0].toPublic().armor();c.passphrase=a;app.saveObject("publicKey-"+b.id_str,f);app.saveObject("privateKey-"+b.id_str,e);app.saveObject("keyOptions-"+b.id_str,c);app.activeUser.publicKey=f;app.activeUser.privateKey=e;app.activeUser.keyOptions=c}else alert("Old passphrase was incorrect.  Please try again");app.renderPrivateKeySettings()},overlay:function(a){a||(a="overlay");$("#"+a).css("display","block")},hideOverlays:function(){$("#overlay").css("display","none");$("#static-overlay").css("display",
"none")},getNameValuePairsFromArgumentList:function(a,b,c){var d=[];a=a.split(b);for(b=0;b<a.length;b++){var e=a[b].split(c);d.push({name:e[0],value:e[1]})}return d},isIOS:function(){return/iPad|iPhone|iPod/.test(navigator.userAgent)&&!window.MSStream},timer:function(a){var b=moment();app.timers[a]?(console.log(a+" took "+b.diff(app.timers[a])+" milliseconds"),delete app.timers[a]):app.timers[a]=b}};
