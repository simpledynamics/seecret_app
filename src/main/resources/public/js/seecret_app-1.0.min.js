...
app.oauthResult?(app.authenticatedUI(),app.maxId=null,app.updateHomeTimeline()):app.unauthenticatedUI()},dmPostData:{},dmMaxId:null,dmSinceId:null,initializeDirectMessagesView:function(){app.setView(app.DIRECT_MESSAGES_VIEW);""==$("#directMessages").html()?app.getAllDirectMessages():app.getNewestDirectMessages()},getNewestDirectMessages:function(){app.setupDMPostData();app.dmSinceId&&(app.dmPostData.since_id=app.dmSinceId);app.getDirectMessages()},getOlderDirectMessages:function(){app.hasFirstDirectMessage||
(app.setupDMPostData(),app.dmPostData.max_id=app.dmMaxId,app.getDirectMessages())},setupDMPostData:function(){app.dmPostData={count:100,full_text:!0,include_entities:!1,skip_status:!0}},getAllDirectMessages:function(){$("#directMessages").empty();app.dmMaxId=null;app.dmSinceId=null;app.hasFirstDirectMessage=!1;app.setupDMPostData();$("#olderDirectMessagesButtonContainer").hide();app.getDirectMessages()},getDirectMessages:function(){$("#friendName").html("");$(".dmMessageDiv").fadeIn();app.updatingDirectMessages=
!0;$("#directMessagesActionsContainer").empty();$("#directMessagesBreadCrumbContainer").empty();$("#friendName").empty();app.overlay();app.oauthResult.get("1.1/direct_messages.json",{data:app.dmPostData}).done(function(a){0<a.length?($("#olderDirectMessagesButtonContainer").show(),console.log("Done getting the messages, got "+a.length),console.log("From "+a[0].id_str+" to "+a[a.length-1].id_str),app.dmSinceId||(console.log("setting the since id to "+a[0].id_str),app.dmSinceId=a[0].id_str),app.processDirectMessagesResponse(a)):
(app.hideOverlays(),app.dmPostData.since_id?alert("No new direct messages"):app.dmPostData.max_id||$("#directMessages").html("You have no direct messages."))}).fail(function(a){app.hideOverlays()})},updateDirectMessagesUI:function(a){app.hideOverlays();app.dmPostData.since_id?$("#directMessages").prepend(Handlebars.getTemplate("direct-messages-template")(a)):$("#directMessages").append(Handlebars.getTemplate("direct-messages-template")(a));app.updatingDirectMessages=!1},publicKeyStartRegex:/^\s*-----BEGIN PGP PUBLIC KEY BLOCK-----\s/,
...
b);app.activeUser.publicKeys=b},hasFirstDirectMessage:!1,processDirectMessagesResponse:function(a){1==a.length&&a[0].id_str==app.dmMaxId?(app.hasFirstDirectMessage||($("#directMessages").append(Handlebars.getTemplate("no-more-direct-messages-template")()),$("#olderDirectMessagesButtonContainer").hide()),app.hasFirstDirectMessage=!0,app.hideOverlays(),app.updatingDirectMessages=!1):0==a.length?($("#directMessages").append(Handlebars.getTemplate("no-seecrets-in-direct-messages-template")()),app.hideOverlays(),
app.updatingDirectMessages=!1):(a=app.processDirectMessages(a,app.getUniqueDMSenders),app.timelineContainsKeys(a)&&app.savePublicKeys(app.getPublicKeyMessagesFromDMList(a)),app.timelineContainsEncryptedMessages(a)?(app.markEncryptedDirectMessages(a),app.decryptDirectMessages(a)):app.updateDirectMessagesUI(a))},markEncryptedDirectMessages:function(a){for(var b in a)this.isPGPMessage(a[b].seecret)&&(a[b].encrypted=!0,a[b].decrypted=!1)},sendPublicKeyMessages:function(a,b){app.postDirectMessages(a,function(){app.trackSentPublicKeys(b)})},
trackSentPublicKeys:function(a){var b=app.activeUser.sentPublicKeys;null==b&&(b={});for(var c in a)b[c]=new Date;app.activeUser.sentPublicKeys=b;app.saveObject("sentPublicKeys-"+app.activeUser.user.id_str,b)},passphrase:null,decryptDirectMessages:function(a){var b=openpgp.key.readArmored(app.activeUser.privateKey.armored).keys[0];if(app.decryptPrivateKey(b))app.verifyMessagesDecrypted(a,b);else{alert("Could not verify your passphrase.  Showing messages undecrypted.");for(var c in a)a[c].encrypted&&
(a[c].text+=" (could not decrypt)");app.updateDirectMessagesUI(a)}},verifyMessagesDecrypted:function(a,b){var c=null,d;for(d in a)if(a[d].encrypted&&0==a[d].decrypted){c=d;break}var e=app.activeUser.user;if(null!=c)if(a[c].sender.id_str==e.id)a[c].decrypted=!0,app.verifyMessagesDecrypted(a,b);else{a[c].decrypted=!0;a[c].decryptionSuccessful=!1;a[c].originaltext=a[c].text;try{var f=openpgp.message.readArmored(a[c].seecret),g={privateKey:b,message:f};if(app.activeUser.publicKeys&&app.activeUser.publicKeys[a[c].sender.id_str]){var l=
openpgp.key.readArmored(app.activeUser.publicKeys[a[c].sender.id_str].key);g.publicKeys=l.keys}try{openpgp.decrypt(g).then(function(b){a[d].decryptionSuccessful=!0;a[d].seecret=b.data;b.signatures&&0<b.signatures.length&&b.signatures[0].valid&&(a[d].signed=!0);app.verifyMessagesDecrypted(a,g.privateKey)},function(b){a[d].text=app.generateUndecryptedMessageContent(a[d]);app.verifyMessagesDecrypted(a,g.privateKey)})}catch(k){console.log("try/catch error decrypting: "+JSON.stringify(k)),a[d].text=app.generateUndecryptedMessageContent(a[d]),
app.verifyMessagesDecrypted(a,b)}}catch(k){console.log("major try/catch error decrypting: "+JSON.stringify(k)),a[d].text=app.generateUndecryptedMessageContent(a[d]),app.verifyMessagesDecrypted(a,b)}}else{(c=app.activeUser.dmSenders)||(c={});for(var h in a)a[h].encrypted&&a[h].sender.id_str!=app.activeUser.id&&(c[a[h].sender.id_str]=!0);app.saveObject("dmSenders-"+app.activeUser.user.id_str,c);app.refreshActiveUserDMSenders();app.updateDirectMessagesUI(a)}},undecryptedMessageContentHandler:null,generateUndecryptedMessageContent:function(a){return Handlebars.getTemplate("undecrypted-message-content-template")(a)},
...
b);app.doScroll=!0}},maxId:null,lastId:null,homeTimeline:!0,userTimelineId:null,updateUserTimeline:function(a,b){b&&(app.maxId=null);app.userTimelineId=a;app.homeTimeline=!1;app.updatingTimeline=!0;var c={data:{}};c.data.count=200;c.data.screen_name=a;c.data.include_rts=!1;null!=app.maxId?c.data.max_id=app.maxId:$("#status-list").empty();app.hideOverlays();app.getUserTimeline(c,app.handleTimelineResponse)},getTimeline:function(a,b){app.oauthResult.get("1.1/statuses/home_timeline.json",a).done(function(a){b(a);
app.hideOverlays()}).fail(app.handleGetTimelineError)},getUserTimeline:function(a,b){$("#twitterUser").val("");app.oauthResult.get("1.1/statuses/user_timeline.json",a).done(function(a){b(a);app.hideOverlays()}).fail(app.handleGetTimelineError)},handleTimelineResponse:function(a){0<a.length?1==a.length&&a[0].id_str==app.maxId?$("#status-list").append(Handlebars.getTemplate("no-more-timeline-message-template")()):app.processTimelineResponse(a):app.handleEmptyTimelineResponse(a)},handleGetTimelineError:function(a){429==
