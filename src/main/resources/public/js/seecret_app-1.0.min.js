...
break;default:app.updateHomeTimeline(),app.setView(app.TIMELINE_VIEW)}},initializeDirectMessagesView:function(){app.setView(app.DIRECT_MESSAGES_VIEW);""==$("#friendsContainer").html()?app.startDirectMessages():alert("dms already exist")},getNewestDirectMessages:function(){app.setupDMPostData();app.dmSinceId&&(app.dmPostData.since_id=app.dmSinceId);app.overlay();app.doDMPost(app.dmPostData,app.handleDMResponse)},setupDMPostData:function(){app.dmPostData={count:app.NUM_DM,full_text:!0,include_entities:!1,
skip_status:!0}},startDirectMessages:function(){app.setupDMPostData();app.overlay();app.doDMPost(app.dmPostData,app.handleDMResponse)},doDMPost:function(a,b){app.updatingDirectMessages=!0;app.timer("direct message post");app.oauthResult.get("1.1/direct_messages.json",{data:a}).done(function(a){app.timer("direct message post");0<a.length?(a.length==app.NUM_DM&&$("#getOlderMessagesButton").show(),app.dmPostData.max_id||(app.dmSinceId=a[0].id_str),b(a)):(app.hideOverlays(),app.dmPostData.since_id?alert("No new direct messages"):
app.dmPostData.max_id||$("#directMessages").html("You have no direct messages."))}).fail(function(a){app.hideOverlays()})},toConversation:function(a){app.setView(app.DIRECT_MESSAGES_VIEW);null==document.getElementById("friendDetails_"+a)?app.getFollowerInfo(a,app.createConversationEntry):app.toggleFriendMessages(a,"friendMessagesToggleLink_"+a)},getFollowerInfo:function(a,b){app.oauthResult.get("1.1/users/show.json?user_id="+a).done(function(a){b(a);$("#friendMessageForm_"+a.id_str).show()}).fail(function(a){console.log("Error getting new follower id "+
JSON.stringify(a))})},createConversationEntry:function(a){var b=app.getObject("publicKeys-"+app.activeUser.user.id_str);b&&(a.publicKey=b[a.id_str]);if(b=app.getObject("sentPublicKeys-"+app.activeUser.user.id_str))a.sentKey=b[a.id_str];$("#friendsContainer").append(Handlebars.getTemplate("conversation-template")(a))},handleDMResponse:function(a){app.timer("handleDMResponse");for(var b in a);1==a.length&&a[0].id_str==app.dmMaxId?(app.hasFirstDirectMessage=!0,app.updatingDirectMessages=!1):0==a.length?
($("#directMessages").append(Handlebars.getTemplate("no-seecrets-in-direct-messages-template")()),app.hideOverlays(),app.updatingDirectMessages=!1):(a=app.processMessageList(a,"sender","dmMaxId"),app.timelineContainsKeys(a)&&app.savePublicKeys(app.getPublicKeyMessagesFromDMList(a)),app.timelineContainsEncryptedMessages(a)?(app.markEncryptedDirectMessages(a),app.decryptDirectMessages(a)):(app.createConversationEntries(a),app.updateDirectMessagesUI(a,friendList)));app.hideOverlays();app.timer("handleDMResponse")},
createConversationEntries:function(a){var b=app.getFriendsFromMessageList(a);if(null!=app.dmPostData.since_id){var c="New messages from ",d="",e;for(e in b)c+=d+b[e].screen_name,d=",";0<a.length&&alert(c);for(e in b)b[e].newMessages=!0,b[e].newMessageCount=8<b[e].message_count?9:b[e].message_count}app.markMessageableFriends(b);app.markFriendsWithSentKeys(b);app.markFriendsWithUnencryptedMessages(b,a);if($("#friendsContainer").is(":empty"))$("#friendsContainer").html(Handlebars.getTemplate("conversations-template")(b));
else{a=[];for(e in b)if(null==document.getElementById("friendDetails_"+b[e].id_str))a.push(b[e]);else{if(c=$("#messageCount_"+b[e].id_str).html())b[e].message_count+=parseInt(c);c=!1;"hide conversation"==$("#friendMessagesToggleLink_"+b[e].id_str).html()&&(c=!0);$("#conversationStatusContainer_"+b[e].id_str).replaceWith(Handlebars.getTemplate("conversation-status-template")(b[e]));c&&($("#friendMessageForm_"+b[e].id_str).show(),$("#friendMessagesToggleLink_"+b[e].id_str).html("hide conversation"))}0<
a.length&&$("#friendsContainer").prepend(Handlebars.getTemplate("conversations-template")(a))}},updateDirectMessagesUI:function(a,b){app.hideOverlays();b=b?b:app.getFriendsFromMessageList(a);app.renderAllFriendMessages(b,a);app.updatingDirectMessages=!1},markMessageableFriends:function(a){var b=app.getObject("publicKeys-"+app.activeUser.user.id_str),c;for(c in a)a[c].publicKey=b[a[c].id_str]},markFriendsWithSentKeys:function(a){var b=app.getObject("sentPublicKeys-"+app.activeUser.user.id_str),c;for(c in b)for(var d in a)c==
a[d].id_str&&(a[d].sentKey=b[c])},markFriendsWithUnencryptedMessages:function(a,b){for(var c in a){a[c].unencryptedMessages=!1;for(var d in b)if(b[d].encrypted&&!b[d].encryptionSuccessful){a[c].unencryptedMessages=!0;break}}},renderAllFriendMessages:function(a,b){for(var c in a)app.renderFriendMessages(a[c].id_str,b)},renderFriendMessages:function(a,b){var c=app.filterMessageListByFriendId(b,a);if(app.dmPostData.since_id){for(var d in c)c[d].new=!0;$("#friendMessages_"+a).prepend(Handlebars.getTemplate("direct-messages-template")(c))}else $("#friendMessages_"+
a).append(Handlebars.getTemplate("direct-messages-template")(c))},getFriendsFromMessageList:function(a){var b={},c;for(c in a)b[a[c].sender.screen_name]||(b[a[c].sender.screen_name]=a[c].sender,b[a[c].sender.screen_name].most_recent=a[c]),b[a[c].sender.screen_name].message_count||(b[a[c].sender.screen_name].message_count=0),b[a[c].sender.screen_name].message_count++;a=[];for(f in b)a.push(b[f]);return a},toggleFriendMessages:function(a){"show conversation"!=$("#friendMessagesToggleLink_"+a).html()?
($("#friendMessagesToggleLink_"+a).html("show conversation"),$("#friendMessageForm_"+a).fadeOut(),$("#friendMessages_"+a).fadeOut()):($("#friendMessagesToggleLink_"+a).html("hide conversation"),$("#friendMessageForm_"+a).fadeIn(),$("#friendMessages_"+a).fadeIn())},filterMessageListByFriendId:function(a,b){var c=[],d;for(d in a)a[d].sender.id_str!=b&&a[d].sender.id_str!=app.activeUser.id_str||c.push(a[d]);return c},getOlderDirectMessages:function(){app.hasFirstDirectMessage||(app.setupDMPostData(),
...
timelineContainsKeys:function(a){for(var b in a)if(app.isPublicKey(a[b].seecret))return!0;return!1},timelineContainsEncryptedMessages:function(a){for(var b in a)if(app.isPGPMessage(a[b].seecret))return!0;return!1},getPublicKeyMessagesFromDMList:function(a){var b=[],c;for(c in a)app.isPublicKey(a[c].seecret)&&b.push(a[c]);return b},savePublicKeys:function(a){var b=app.activeUser.publicKeys;null==b&&(b={});var c={},d;for(d in a)a[d].sender.id_str!=app.activeUser.user.id&&(c[a[d].sender.id_str]||(c[a[d].sender.id_str]=
{key:a[d].seecret,screen_name:a[d].sender.screen_name,id_str:a[d].sender.id_str,profile_image_url:a[d].sender.profile_image_url,receiveDate:a[d].created_at}));for(var e in c)b[e]||$("#friendKeyContainer_"+e).html("Just sent you their key!"),b[e]=c[e];app.saveObject("publicKeys-"+app.activeUser.user.id_str,b);app.activeUser.publicKeys=b},markEncryptedDirectMessages:function(a){for(var b in a)this.isPGPMessage(a[b].seecret)&&(a[b].encrypted=!0,a[b].decrypted=!1)},sendPublicKeyMessages:function(a,b){app.postDirectMessages(a,
function(){app.trackSentPublicKeys(b)})},trackSentPublicKeys:function(a){var b=app.activeUser.sentPublicKeys;null==b&&(b={});for(var c in a)b[c]=new Date;app.activeUser.sentPublicKeys=b;app.saveObject("sentPublicKeys-"+app.activeUser.user.id_str,b)},decryptDirectMessages:function(a){app.timer("decryptDirectMessages");var b=openpgp.key.readArmored(app.activeUser.privateKey.armored).keys[0];if(app.decryptPrivateKey(b))app.verifyMessagesDecrypted(a,b);else{alert("Could not verify your passphrase.  Showing messages undecrypted.");
for(var c in a)a[c].encrypted&&(a[c].text+=" (could not decrypt)");app.updateDirectMessagesUI(a)}},verifyMessagesDecrypted:function(a,b){var c=null,d;for(d in a)if(a[d].encrypted&&0==a[d].decrypted){c=d;break}var e=app.activeUser.user;if(null!=c)if(a[c].sender.id_str==e.id)a[c].decrypted=!0,app.verifyMessagesDecrypted(a,b);else{a[c].decrypted=!0;a[c].decryptionSuccessful=!1;a[c].originaltext=a[c].text;try{var g=openpgp.message.readArmored(a[c].seecret),h={privateKey:b,message:g};if(app.activeUser.publicKeys&&
app.activeUser.publicKeys[a[c].sender.id_str]){var k=openpgp.key.readArmored(app.activeUser.publicKeys[a[c].sender.id_str].key);h.publicKeys=k.keys}try{openpgp.decrypt(h).then(function(b){a[d].decryptionSuccessful=!0;a[d].seecret=b.data;b.signatures&&0<b.signatures.length&&b.signatures[0].valid&&(a[d].signed=!0);app.verifyMessagesDecrypted(a,h.privateKey)},function(b){a[d].text=app.generateUndecryptedMessageContent(a[d]);app.verifyMessagesDecrypted(a,h.privateKey)})}catch(m){console.log("try/catch error decrypting: "+
JSON.stringify(m)),a[d].text=app.generateUndecryptedMessageContent(a[d]),app.verifyMessagesDecrypted(a,b)}}catch(m){console.log("major try/catch error decrypting: "+JSON.stringify(m)),a[d].text=app.generateUndecryptedMessageContent(a[d]),app.verifyMessagesDecrypted(a,b)}}else{(c=app.activeUser.dmSenders)||(c={});for(var l in a)a[l].encrypted&&a[l].sender.id_str!=app.activeUser.id&&(c[a[l].sender.id_str]=!0);app.saveObject("dmSenders-"+app.activeUser.user.id_str,c);app.refreshActiveUserDMSenders();
...
(c=0));return c},processMessageList:function(a,b,c){var d=function(a){return a[b]?a[b].id_str:null},e=app.getUniqueInstancesFromList(a,d),g=app.getMessageIndexes(a,e,d);a=app.dechainifyMessages(a,g,b);app[c]=app.lastProcessedStatusId=a[a.length-1].id_str;var g=0,h;for(h in e){var e=0,k;for(k in a)d(a[k])==h&&app.seecret_engine.hasSeecretContent(a[k].text)&&(app.seecret_engine.isEnvelopeStart(a[k].text)&&(e=k),app.seecret_engine.isEnvelopeEnd(a[k].text)&&(e=0));0<e&&0==g?g=e:0<e&&e<g&&(g=e)}d=g;0<
d&&(app[c]=a[d].id_str);a=app.unhideAndDecompressSeecretsInList(a,!0);for(k in a)a[k][b]&&a[k][b].profile_image_url&&(a[k][b].profile_image_url=a[k][b].profile_image_url.replace(app.httpsReplaceRegex,"https:"));return a},processTimelineWithFollowerInfo:function(a){var b=null;app.dmPost&&app.dmPost.since_id&&app.dmMaxId&&(b=app.dmMaxId);a=app.processMessageList(a,"user","maxId");b&&(app.dmMaxId=b);0<a.length?$("#status-list").append(Handlebars.getTemplate("status-template")(a)):document.getElementById("showOnlySeecrets").checked&&
$("#status-list").append(Handlebars.getTemplate("no-seecrets-in-timeline-segment-template")());$(".basicTweet").fadeIn();app.updatingTimeline=!1;app.userTimelineId?app.renderUserTimelineHeader(app.userTimelineId):$("#timelineName").html("")},renderUserTimelineHeader:function(a){$("#timelineName").html("for "+a+" &nbsp;&nbsp;&nbsp;<a href='javascript:app.initTimeline();app.updateHomeTimeline()'>Back to my timeline</a>")},dechainifyMessages:function(a,b,c){for(var d in b)if(0<b[d].length){var e=function(a){return a&&
a.text&&"string"==typeof a.text&&a[c]&&a[c].id_str==d?a:null},g=0,h;for(h in b[d]){var k=app.seecret_engine.dechainify(a,{chainSegmentMatcher:e,chainSegmentContentFinder:this.chainSegmentContentFinder,startIndex:b[d][h],withCovertexts:!0});k&&""!=k.seecret&&(a[b[d][h]].seecret_envelope=k.seecret,a[b[d][h]].covertexts=k.covertexts);g++}}return a},chainSegmentContentFinder:function(a){return a.text},tweetSenderGetter:function(a){return a.user?a.user.id_str:null},dmSenderGetter:function(a){return a.user?
a.user.id_str:null},getMessageIndexes:function(a,b,c){var d={},e;for(e in b)d[e]=app.seecret_engine.getOrdinalIndexes(a,{chainSegmentMatcher:function(a){return c(a)==b[e]?a:null},chainSegmentContentFinder:this.chainSegmentContentFinder});return d},getSeecretMessageIndexesByUser:function(a,b){var c={},d;for(d in b)c[d]=app.seecret_engine.getOrdinalIndexes(a,{chainSegmentMatcher:function(a){return a&&a.text&&"string"==typeof a.text&&a.user&&a.user.id_str==b[d]?a:null},chainSegmentContentFinder:this.chainSegmentContentFinder});
return c},getSeecretMessageIndexesBySender:function(a,b){var c={},d;for(d in b)c[d]=app.seecret_engine.getOrdinalIndexes(a,{chainSegmentMatcher:function(a){return a&&a.text&&"string"==typeof a.text&&a.sender&&a.sender.id_str==b[d]?a:null},chainSegmentContentFinder:this.chainSegmentContentFinder});return c},getUniqueTweeters:function(a){return app.getUniqueInstancesFromList(a,app.getTweeterId)},getUniqueDMSenders:function(a){return app.getUniqueInstancesFromList(a,app.getDMSenderId)},getTweeterId:function(a){return a.user.id_str},
getDMSenderId:function(a){return a.sender.id_str},getUniqueInstancesFromList:function(a,b){var c={},d;for(d in a){var e=b(a[d]);e&&(c[e]=e)}return c},generateTimelinePostVals:function(){var a={data:{}};a.data.count=200;null!=app.maxId&&(a.data.max_id=app.maxId);return a},initTimeline:function(){app.maxId=null;app.homeTimeline=!0;app.userTimelineId=null;app.updateHomeTimeline()},updateHomeTimeline:function(a){$("#twitterUser").val("");a&&(app.maxId=null);app.homeTimeline=!0;$("#timelineName").empty();
app.updatingTimeline=!0;a=app.generateTimelinePostVals();a.data.max_id||($("#status-list").empty(),app.lastProcessedStatusId=null);app.overlay();app.timer("Get timeline");app.getTimeline(a,app.handleTimelineResponse)},prepareUpdateMessage:function(){var a=$("#textarea-statusUpdate").val(),a=app.hideAndCompress(a),b=null;app.useAutogeneratedScaffolding()||(b=app.getScaffolds());a=app.seecret_engine.chainify(a,b);a.reverse();return a},updateStatus:function(){var a=this.prepareUpdateMessage();this.postChain(a)},
postChain:function(a){this.currentMessageChain=a;this.chainCount=0;this.postChainItem()},postChainItem:function(){this.chainCount<this.currentMessageChain.length?app.oauthResult.post("1.1/statuses/update.json",{data:{status:this.currentMessageChain[this.chainCount++]}}).done(function(a){setTimeout(function(){app.postChainItem()},50)}).fail(function(a){}):($("#status-list").empty(),$("#scaffoldingWarningContainer").empty(),$("#scaffoldingStatus").empty(),$("#textarea-statusUpdate").val(""),$("#textarea-statusUpdate").val(""),
$("#scaffoldsContainer").html(Handlebars.getTemplate("scaffolds-template")()),$("#scaffoldingContainer > div > div > input").val(""),app.maxId=null,this.currentMessageChain=[],this.chainCount=0,app.updateHomeTimeline(),app.setView(app.TIMELINE_VIEW))},authenticatedUI:function(){app.authenticatedHeader();app.authenticatedMenu();app.setView(app.TIMELINE_VIEW)},unauthenticatedUI:function(){$("#status-list").empty();$("#userLabel").empty();$("#userProfileImg").hide();$("#directMessages").empty();$("#publicKeys").empty();
$("#directMessagesActionsContainer").val("");$("#textarea-statusUpdate").val("");$("#importPrivateKeyText").val("");$("#importPrivateKeyPassphrase").val("");$("#privateKeyTextarea").val("");$("#privateKeyPassphrase").val("");$("#privateKeyCopyarea").empty();$("#publicKeysDisplayArea").empty();app.unauthenticatedHeader();$("#privateMessagesButton").hide();$("#timelineButton").hide();app.unauthenticatedMenu();app.setView(app.ABOUT_VIEW);app.hideOverlays()},authenticatedMenu:function(){$("#privateMessagesButton").show();
$("#timelineButton").show();$("#settingsButton").show();$("#logoutButton").show();$("#hashesButton").show();$("#publicKeysButton").show();$("#aboutButton").show()},unauthenticatedMenu:function(){$("#privateMessagesButton").hide();$("#timelineButton").hide();$("#settingsButton").hide();$("#logoutButton").hide();$("#publicKeysButton").hide()},authenticatedHeader:function(){app.activeUser&&app.activeUser.user&&(console.log("app.active user = "+JSON.stringify(app.activeUser.user)),app.activeUser.user.profile_image_url=
app.activeUser.user.profile_image_url.replace(app.httpsReplaceRegex,"https:"),$("#loginContainer").html(Handlebars.getTemplate("authenticated-header-template")(app.activeUser.user)))},unauthenticatedHeader:function(){$("#loginContainer").html(Handlebars.getTemplate("unauthenticated-header-template")())},hideAllViews:function(){$("#timeline").hide();$("#statusMessage").hide();$("#privateMessages").hide();$("#settings").hide();$("#publicKeys").hide();$("#about").hide();$("#hashes").hide()},setMenuState:function(a){$.each($(".menuLink"),
function(a,c){$(this).css("background","");$(this).css("color","")});$("#"+a).css("background","#e3e3e3");$("#"+a).css("color","#333");app.openClosePanel("close")},setView:function(a){app.doScroll=!1;app.hideAllViews();$("#"+a).fadeIn(function(){app.doScroll=!0}).css("display","block");app.setMenuState(a+"Button")},renderPublicKeys:function(){var a=app.activeUser.publicKeys,b=[],c=0,d;for(d in a)a[d].displayDate=moment(a[d].receiveDate).format("MMMM Do YYYY, h:mm:ss a"),b.push(a[d]),c++;0<c?(a=Handlebars.getTemplate("public-keys-template")({keys:b}),
$("#publicKeysDisplayArea").html(a)):$("#publicKeysDisplayArea").html("No public keys")},copySenderPublicKeyToClipboard:function(a){var b=app.activeUser.publicKeys;b[a]&&(executeCopy(b[a].key),alert(b[a].screen_name+"'s key has been copied to your clipboard"))},copyContentsToClipboard:function(a){executeCopy($("#"+a).html())},showPrivateKeyManager:function(){app.doScroll=!1;$("#directMessagesActionsContainer").html(Handlebars.getTemplate("private-key-template")());$("#directMessagesActionsContainer").show()},
openClosePanel:function(a){"open"==a?$("#left-panel").css("left",0):$("#left-panel").css("left","-14em")},processScaffoldField:function(a){a=$("#scaffold"+a);val=a.val();for(var b=val.replace(/\s{2,}/g," ");" "==b.substr(0,1);)b=b.substr(1);b=b.substr(0,100);b!=val&&a.val(b)},useAutogeneratedScaffolding:function(){return!1},hideAndCompress:function(a){var b=app.seecret_engine.hide(a,app.seecret_engine.config.CONTENT_TYPES.PLAIN),b=app.seecret_engine.envelope(b,app.seecret_engine.config.CONTENT_TYPES.PLAIN);
a=seecret_compression.compress(a);a=app.seecret_engine.hide(a,app.seecret_engine.config.CONTENT_TYPES.NUMBERS_ARRAY);a=app.seecret_engine.envelope(a,app.seecret_engine.config.CONTENT_TYPES.NUMBERS_ARRAY);return b.length>a.length?a:b},processScaffolding:function(){var a=$("#textarea-statusUpdate").val();if(!app.useAutogeneratedScaffolding()){var b=!1,a=app.hideAndCompress(a),c=app.getScaffolds(),d=0,e=0;if(1==c.length)if(2<=c[0].trim().length&&c[0].trim().length+a.length<app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH)b=
!0,d=app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH-a.length-c[0].trim().length;else{if(2>c[0].trim().length){app.makeUpdateFormPending();return}e=app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH-(c[0].trim().length+a.length);e*=-1}else{var g=scaffoldLength=0;for(each in c)2<=c[each].trim().length&&(scaffoldLength+=c[each].trim().length,g++);scaffoldLength+a.length<app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH*g?(b=!0,d=app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH*g-(scaffoldLength+
a.length)):(e=app.seecret_engine.config.MAX_CHAIN_SEGMENT_LENGTH*g-(scaffoldLength+a.length),e*=-1)}b?app.makeUpdateFormReady(d):app.makeUpdateFormNotReady(e)}},newVisibleMessageField:function(){var a=app.getScaffolds();app.addScaffoldField(a.length);app.processScaffolding()},addScaffoldField:function(a,b){var c={scaffoldId:a,val:b,scaffoldCount:a+1};$("#scaffoldsContainer").append(Handlebars.getTemplate("scaffold-template")(c))},makeUpdateFormReady:function(a){$("#submitButtonContainer").show();
var b={};b.availableCharacters=a;$("#scaffoldingStatus").html(Handlebars.getTemplate("scaffold-status-good-template")(b))},makeUpdateFormNotReady:function(a){$("#submitButtonContainer").hide();var b={};b.spillover=a;$("#scaffoldingStatus").html(Handlebars.getTemplate("scaffold-status-spillover-template")(b))},makeUpdateFormPending:function(){$("#submitButtonContainer").hide();$("#scaffoldingStatus").html(Handlebars.getTemplate("scaffold-status-incomplete-template")())},removeScaffold:function(a){var b=
app.getScaffolds(),c=[],d;for(d in b)d!=a&&c.push(b[d]);app.renderScaffolds(c);app.processScaffolding()},renderScaffolds:function(a){$("#scaffoldsContainer").html("");for(var b=0,c=0;c<a.length;c++)app.addScaffoldField(b++,a[c]);$("#scaffoldRemover0").remove()},getScaffolds:function(){for(var a=[],b=document.getElementById("scaffold0");b;)a.push(b.value.trim()),b=document.getElementById("scaffold"+a.length);return a},toggleScaffolds:function(){document.getElementById("scaffoldingOption").checked?
($("#scaffoldingStatus").html(""),$("#scaffoldsContainer").html(""),$("#scaffoldingWarningContainer").hide(),$("#scaffoldingContainer").hide(),$("#submitButtonContainer").show()):($("#scaffoldingStatus").html(""),$("#scaffoldsContainer").html(""),$("#scaffoldingContainer").show(),$("#scaffoldingWarningContainer").html(""),$("#submitButtonContainer").hide(),app.addScaffoldField(0),app.processScaffolding())},getScaffoldTotal:function(a){for(var b=0,c=0;c<a.length;c++)b+=a[c].trim().length;return b},
showCopyPrivateKey:function(a){"block"==$("#privateKeyCopyarea").css("display")?($(a).html("Show"),$("#privateKeyCopyarea").hide(),$("#privateKeyCopyarea").html("")):($(a).html("Hide"),app.renderPrivateKeySettings())},toggleDeleteSettings:function(a){"block"==$("#deleteKeyForm").css("display")?($(a).html("Show"),$("#deleteKeyForm").hide()):($(a).html("Hide"),$("#deleteKeyForm").show())},renderPrivateKeySettings:function(){var a=app.activeUser.privateKey,b=app.activeUser.publicKey,c=app.activeUser.keyOptions,
d={};app.activeUser&&app.activeUser.privateKey&&(d.privateKey=a,d.privateKey.dateCreatedDisplay=(new moment(d.privateKey.dateCreated)).format("MMMM Do YYYY, h:mm:ss a"),d.publicKey=b,d.keyOptions=c);a=Handlebars.getTemplate("user-keys-template")(d);$("#privateKeyCopyarea").html(a);$("#privateKeyCopyarea").fadeIn()},showPublicKeys:function(a){"block"==$("#publicKeysDisplayArea").css("display")?($(a).html("Show"),$("#publicKeysDisplayArea").hide()):($(a).html("Hide"),app.renderPublicKeys(),$("#publicKeysDisplayArea").fadeIn())},
showImportPrivateKey:function(a){"block"==$("#importForm").css("display")?($(a).html("Show"),$("#importForm").hide()):($(a).html("Hide"),$("#importForm").show())},showGeneratePrivateKey:function(a){"block"==$("#generateForm").css("display")?($(a).html("Show"),$("#generateForm").hide()):($(a).html("Hide"),$("#generateForm").show())},saveObject:function(a,b){localStorage[a]=JSON.stringify(b)},getObject:function(a){return app.getLocalObject(a)},getLocalObject:function(a){return localStorage[a]?JSON.parse(localStorage[a]):
null},saveLocalObject:function(a,b){localStorage[a]=JSON.stringify(b)},deleteSavedItem:function(a){delete localStorage[a]},importPrivateKey:function(){var a=app.activeUser.user,b=$("#importPrivateKeyText").val(),c=new Date,d=openpgp.key.readArmored(b);if(!d||!d.keys||1>d.keys.length)alert("The key is invalid");else{var e=openpgp.key.readArmored(b).keys[0],g=$("#importPrivateKeyPassphrase").val();e.decrypt(g)?(g=$("#importPrivateKeyPassphrase").val(),$("#importPrivateKeyText").val(""),$("#importPrivateKeyPassphrase").val(""),
d=app.makeKeyOptionsForUser(a),d.passphrase=g,b={armored:b,dateCreated:c},c={armored:e.toPublic().armor(),dateCreated:c},app.activeUser.privateKey=b,app.activeUser.publicKey=c,app.activeUser.keyOptions=d,app.saveObject("privateKey-"+a.id_str,b),app.saveObject("publicKey-"+a.id_str,c),app.saveObject("keyOptions-"+a.id_str,d),app.activeUser.privateKey=b,app.activeUser.publicKey=c,app.activeUser.keyOptions=d,app.renderPrivateKeySettings(),confirm("Successfully imported the private key. Send your new key to the contacts who have the old one?")&&
app.resendPublicKeys()):alert("Could not decrypt the key with the passphrase you entered.  Please try again")}},deleteSavedPassphrase:function(){if(confirm("Are you sure?")){var a=app.activeUser.user,b=app.activeUser.keyOptions;b.passphrase="";app.activeUser.keyOptions=b;app.saveObject("keyOptions-"+a.id_str,b)}app.renderPrivateKeySettings()},changePassphrase:function(){var a=prompt("Enter new passphrase");if(null!=a){var b=prompt("Enter it again");null!=b&&(a==b?(alert("Changing your passphrase to "+
b+"\n we urge you to delete it from this device once you've got it memorized or saved elsewhere."),app.setNewPassphraseOnPrivateKey(a)):alert("Your new passphrase did not match, please try again"))}app.renderPrivateKeySettings()},setNewPassphraseOnPrivateKey:function(a){var b=app.activeUser.user,c=app.activeUser.keyOptions,d=openpgp.key.readArmored(app.activeUser.privateKey.armored);if(app.decryptPrivateKey(d.keys[0])){d.keys[0].primaryKey.encrypt(a);d.keys[0].subKeys&&0<d.keys[0].subKeys.length&&
d.keys[0].subKeys[0]&&d.keys[0].subKeys[0].subKey.encrypt(a);var e={};e.dateCreated=new Date;e.armored=d.keys[0].armor();var g={};g.dateCreated=new Date;g.armored=d.keys[0].toPublic().armor();c.passphrase=a;app.saveObject("publicKey-"+b.id_str,g);app.saveObject("privateKey-"+b.id_str,e);app.saveObject("keyOptions-"+b.id_str,c);app.activeUser.publicKey=g;app.activeUser.privateKey=e;app.activeUser.keyOptions=c}else alert("Old passphrase was incorrect.  Please try again");app.renderPrivateKeySettings()},
overlay:function(a){a||(a="overlay");$("#"+a).css("display","block")},hideOverlays:function(){$("#overlay").css("display","none");$("#static-overlay").css("display","none")},getNameValuePairsFromArgumentList:function(a,b,c){var d=[];a=a.split(b);for(var e in a)b=a[e].split(c),d.push({name:b[0],value:b[1]});return d},isIOS:function(){return/iPad|iPhone|iPod/.test(navigator.userAgent)&&!window.MSStream},timer:function(a){var b=moment();app.timers[a]?(console.log(a+" took "+b.diff(app.timers[a])+" milliseconds"),
