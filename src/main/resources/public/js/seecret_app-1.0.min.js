...
a.length?(app.processDirectMessagesResponse(a),app.updateDirectMesssagesBreadcrumbs(null)):(app.hideOverlays(),$("#directMessages").html("No direct messages."))}).fail(function(a){app.hideOverlays()})},updateDirectMessagesUI:function(a){app.hideOverlays();$("#getMessagesButton2").remove();$("#directMessages").append(Handlebars.templates["direct-messages-template"](a));$("#olderDirectMessagesButtonContainer").show();app.updatingDirectMessages=!1},publicKeyStartRegex:/^\s*-----BEGIN PGP PUBLIC KEY BLOCK-----\s/,
...
b);app.activeUser.publicKeys=b},processDirectMessagesResponse:function(a){1==a.length&&a[0].id_str==app.dmMaxId?($("#directMessages").append(Handlebars.templates["no-more-direct-messages-template"]()),app.hideOverlays(),app.updatingDirectMessages=!1):0==a.length?($("#directMessages").append(Handlebars.templates["no-seecrets-in-direct-messages-template"]()),app.hideOverlays(),app.updatingDirectMessages=!1):(a=app.processDirectMessages(a,app.getUniqueDMSenders),app.timelineContainsKeys(a)&&app.savePublicKeys(app.getPublicKeyMessagesFromDMList(a)),
...
app.generateUndecryptedMessageContent(a[d]),app.verifyMessagesDecrypted(a,b)}}else{(c=app.activeUser.dmSenders)||(c={});for(var h in a)a[h].encrypted&&a[h].sender.id_str!=app.activeUser.id&&(c[a[h].sender.id_str]=!0);app.saveObject("dmSenders-"+app.activeUser.user.id_str,c);app.refreshActiveUserDMSenders();app.updateDirectMessagesUI(a)}},undecryptedMessageContentHandler:null,generateUndecryptedMessageContent:function(a){return Handlebars.templates["undecrypted-message-content-template"](a)},showPublicKeyManager:function(a,
b){app.doScroll=!1;var c={};c.receiver_name=b;c.receiver_id=a;$("#directMessagesActionsContainer").html(Handlebars.templates["public-key-template"](c))},showWaitingForPublicKeyView:function(a,b){app.doScroll=!1;var c={};c.receiver_name=b;c.receiver_id=a;c.invite_date=moment(app.activeUser.sentPublicKeys[a]).format("MMMM Do YYYY, h:mm:ss a");$("#directMessagesActionsContainer").html(Handlebars.templates["public-key-pending-template"](c))},showDirectMessageForm:function(a,b){app.doScroll=!1;var c={receiverId:a,
receiverName:b};$("#directMessagesActionsContainer").html(Handlebars.templates["direct-message-template"](c));app.doScroll=!1},makeKeyOptionsForUser:function(a){var b=app.generatePassword(),c=1024;openpgp.util.getWebCryptoAll()&&(c=2048);return{numBits:c,userIds:[{name:a.name,email:a.screen_name+"@seecretApp.seecret"}],passphrase:b}},generatePassword:function(){for(var a="",b=0;50>b;b++)a+=app.getValidPasswordCharacter();return a},getValidPasswordCharacter:function(){for(;;){var a=Math.floor(206*
Math.random())+48;if(!(173==a||92==a||58<=a&&64>=a||127<=a&&160>=a))return String.fromCharCode(a)}},validateNewPrivateKey:function(a){if(1>$("#"+a).val().length)return $("#"+a+"Error").html("You must have a passphrase for your key of at least one character"),!1;$("#"+a+"Error").html("");if(confirm("Are you sure?")){var b=$("#"+a).val(),c=1==document.getElementById(a+"Cache").checked;app.hideAllViews();$("#"+app.STATUS_MESSAGE_VIEW).show();app.generatePrivateKeyPair(b,c,function(){$("#"+a).val("");
confirm("Send your new key to the contacts who have the old one?")&&app.resendPublicKeys();$("#directMessagesActionsContainer").empty();app.renderPrivateKeySettings();app.setView(app.SETTINGS_VIEW)})}},resendPublicKeys:function(){var a=app.activeUser.sentPublicKeys,b=app.activeUser.dmSenders;if(null!=a||null!=b){var c=[],d={},e;for(e in a)d[e]=!0;for(e in b)d[e]=!0;var a=0,f;for(f in d){a++;var b=app.generatePublicKeyMessages(f),g;for(g in b)c.push({receiverId:f,text:b[g]})}0<c.length&&app.sendPublicKeyMessages(c,
d)}},generatePrivateKeyPair:function(a,b,c){var d=app.activeUser.user,e=app.makeKeyOptionsForUser(d);e.passphrase=a;d.id_str||(e.userId=d.id_str);null!=d&&(a=new Date,console.log("start generating key pair : "+a.getSeconds()+":"+a.getMilliseconds()),openpgp.config.debug=!0,openpgp.generateKey(e).then(function(a){var d=new Date;console.log("done generating key pair : "+d.getSeconds()+":"+d.getMilliseconds());app.generatePrivateKeyPairSuccess(a,e,b,c)}).catch(function(a){console.log("Caught an error generatinga key pair");
...
$("#selectedReceiverName").val(b);var c={};c.name=b;c.id_str=a;var d=app.activeUser.publicKeys,e;for(e in d)d[e].id_str==a&&(c.publicKey=d[e],c.publicKey.displayDate=(new moment(c.publicKey.receiveDate)).format("MMMM Do YYYY, h:mm:ss a"));$("#friendName").html(Handlebars.templates["direct-message-friend-name"](c));app.checkDirectMessageStatus();$(".dmMessageDiv").hide();$(".dm_"+a).fadeIn()},updateDirectMesssagesBreadcrumbs:function(a){$("#directMessagesBreadCrumbContainer").html(Handlebars.templates["direct-message-breadcrumb-template"](a))},
...
a;app.homeTimeline=!1;app.updatingTimeline=!0;var c={data:{}};c.data.count=200;c.data.screen_name=a;c.data.include_rts=!1;null!=app.maxId?c.data.max_id=app.maxId:$("#status-list").empty();app.hideOverlays();app.getUserTimeline(c,app.handleTimelineResponse)},getTimeline:function(a,b){app.oauthResult.get("1.1/statuses/home_timeline.json",a).done(function(a){b(a);app.hideOverlays()}).fail(app.handleGetTimelineError)},getUserTimeline:function(a,b){app.oauthResult.get("1.1/statuses/user_timeline.json",
a).done(function(a){b(a);app.hideOverlays()}).fail(app.handleGetTimelineError)},handleTimelineResponse:function(a){0<a.length?1==a.length&&a[0].id_str==app.maxId?$("#status-list").append(Handlebars.templates["no-more-timeline-message-template"]()):app.processTimelineResponse(a):app.handleEmptyTimelineResponse(a)},handleGetTimelineError:function(a){429==a.status?alert("Twitter rate limit exceeded.  You must wait 15 minutes to make more requests from this app."):404==a.status?alert("Twitter says that screen name does not exist."):
...
postChainItem:function(){this.chainCount<this.currentMessageChain.length?app.oauthResult.post("1.1/statuses/update.json",{data:{status:this.currentMessageChain[this.chainCount++]}}).done(function(a){setTimeout(function(){app.postChainItem()},50)}).fail(function(a){}):($("#status-list").empty(),$("#scaffoldingWarningContainer").empty(),$("#scaffoldingStatus").empty(),$("#textarea-statusUpdate").val(""),$("#textarea-statusUpdate").val(""),$("#scaffoldsContainer").html(Handlebars.templates["scaffolds-template"]()),
...
$("#hashesButton").show();$("#publicKeysButton").show();$("#aboutButton").show()},unauthenticatedMenu:function(){$("#privateMessageButton").hide();$("#timelineButton").hide();$("#settingsButton").hide();$("#logoutButton").hide();$("#publicKeysButton").hide()},authenticatedHeader:function(){app.activeUser&&app.activeUser.user&&(app.activeUser.user.profile_image_url=app.activeUser.user.profile_image_url.replace(app.httpsReplaceRegex,"https:"),$("#loginContainer").html(Handlebars.templates["authenticated-header-template"](app.activeUser.user)))},
unauthenticatedHeader:function(){$("#loginContainer").html(Handlebars.templates["unauthenticated-header-template"]())},hideAllViews:function(){$("#timeline").hide();$("#statusMessage").hide();$("#privateMessages").hide();$("#settings").hide();$("#publicKeys").hide();$("#about").hide();$("#hashes").hide()},setMenuState:function(a){$.each($(".menuLink"),function(a,c){$(this).css("background","");$(this).css("color","")});$("#"+a).css("background","#e3e3e3");$("#"+a).css("color","#333");app.openClosePanel("close")},
setView:function(a){app.doScroll=!1;app.hideAllViews();$("#"+a).fadeIn(function(){app.doScroll=!0}).css("display","block");app.setMenuState(a+"Button")},renderPublicKeys:function(){var a=app.activeUser.publicKeys,b=[],c=0,d;for(d in a)a[d].displayDate=moment(a[d].receiveDate).format("MMMM Do YYYY, h:mm:ss a"),b.push(a[d]),c++;0<c?(a=Handlebars.templates["public-keys-template"]({keys:b}),$("#publicKeysDisplayArea").html(a)):$("#publicKeysDisplayArea").html("No public keys")},copySenderPublicKeyToClipboard:function(a){var b=
app.activeUser.publicKeys;b[a]&&(executeCopy(b[a].key),alert(b[a].screen_name+"'s key has been copied to your clipboard"))},copyContentsToClipboard:function(a){executeCopy($("#"+a).html())},showPrivateKeyManager:function(){app.doScroll=!1;$("#directMessagesActionsContainer").html(Handlebars.templates["private-key-template"]());$("#directMessagesActionsContainer").show()},openClosePanel:function(a){"open"==a?$("#left-panel").css("left",0):$("#left-panel").css("left","-14em")},processScaffoldField:function(a){a=
...
newVisibleMessageField:function(){var a=app.getScaffolds();app.addScaffoldField(a.length);app.processScaffolding()},addScaffoldField:function(a,b){var c={scaffoldId:a,val:b,scaffoldCount:a+1};$("#scaffoldsContainer").append(Handlebars.templates["scaffold-template"](c))},makeUpdateFormReady:function(a){$("#submitButtonContainer").show();var b={};b.availableCharacters=a;$("#scaffoldingStatus").html(Handlebars.templates["scaffold-status-good-template"](b))},makeUpdateFormNotReady:function(a){$("#submitButtonContainer").hide();
var b={};b.spillover=a;$("#scaffoldingStatus").html(Handlebars.templates["scaffold-status-spillover-template"](b))},makeUpdateFormPending:function(){$("#submitButtonContainer").hide();$("#scaffoldingStatus").html(Handlebars.templates["scaffold-status-incomplete-template"]())},removeScaffold:function(a){var b=app.getScaffolds(),c=[],d;for(d in b)d!=a&&c.push(b[d]);app.renderScaffolds(c);app.processScaffolding()},renderScaffolds:function(a){$("#scaffoldsContainer").html("");for(var b=0,c=0;c<a.length;c++)app.addScaffoldField(b++,
...
$("#deleteKeyForm").css("display")?($(a).html("Show"),$("#deleteKeyForm").hide()):($(a).html("Hide"),$("#deleteKeyForm").show())},renderPrivateKeySettings:function(){var a=app.activeUser.privateKey,b=app.activeUser.publicKey,c=app.activeUser.keyOptions,d={};app.activeUser&&app.activeUser.privateKey&&(d.privateKey=a,d.privateKey.dateCreatedDisplay=(new moment(d.privateKey.dateCreated)).format("MMMM Do YYYY, h:mm:ss a"),d.publicKey=b,d.keyOptions=c);a=Handlebars.templates["user-keys-template"](d);$("#privateKeyCopyarea").html(a);
$("#privateKeyCopyarea").fadeIn()},showPublicKeys:function(a){"block"==$("#publicKeysDisplayArea").css("display")?($(a).html("Show"),$("#publicKeysDisplayArea").hide()):($(a).html("Hide"),app.renderPublicKeys(),$("#publicKeysDisplayArea").fadeIn())},showImportPrivateKey:function(a){"block"==$("#importForm").css("display")?($(a).html("Show"),$("#importForm").hide()):($(a).html("Hide"),$("#importForm").show())},showGeneratePrivateKey:function(a){"block"==$("#generateForm").css("display")?($(a).html("Show"),
$("#generateForm").hide()):($(a).html("Hide"),$("#generateForm").show())},saveObject:function(a,b){localStorage[a]=JSON.stringify(b)},getObject:function(a){return app.getLocalObject(a)},getLocalObject:function(a){return localStorage[a]?JSON.parse(localStorage[a]):null},saveLocalObject:function(a,b){localStorage[a]=JSON.stringify(b)},deleteSavedItem:function(a){delete localStorage[a]},privateKeyUpdateStatus:{},importPrivateKey:function(){var a=app.activeUser.user,b=$("#importPrivateKeyText").val(),
...
[];a=a.split(b);for(var e in a)b=a[e].split(c),d.push({name:b[0],value:b[1]});return d},isIOS:function(){return/iPad|iPhone|iPod/.test(navigator.userAgent)&&!window.MSStream}};
